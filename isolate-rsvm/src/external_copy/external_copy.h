...
using array_buffer_vector_t =
    std::vector<std::unique_ptr<class ExternalCopyArrayBuffer>>;
using shared_buffer_vector_t =
    std::vector<std::unique_ptr<class ExternalCopySharedArrayBuffer>>;
...
class ExternalCopy : public Transferable {
 public:
   ...
   */
  static auto Copy(v8::Local<v8::Value> value,
                   bool transfer_out = false,
                   ArrayRange transfer_list = {})
      -> std::unique_ptr<ExternalCopy>;

  /**
   * If you give this a primitive v8::Value (except Symbol) it will return a
   * ExternalCopy for you. Otherwise it returns nullptr. This is used to
   * automatically move simple values between isolates where it is possible to
   * do so perfectly.
   */
  static auto CopyIfPrimitive(v8::Local<v8::Value> value)
      -> std::unique_ptr<ExternalCopy>;
  static auto CopyThrownValue(v8::Local<v8::Value> value)
      -> std::unique_ptr<ExternalCopy>;
...
class ExternalCopyAnyBuffer : public ExternalCopy {
 public:
  explicit ExternalCopyAnyBuffer(
      std::shared_ptr<v8::BackingStore> backing_store)
      : backing_store{std::move(backing_store)} {}
  auto Acquire() const -> std::shared_ptr<v8::BackingStore> {
...
class ExternalCopyArrayBuffer : public ExternalCopyAnyBuffer {
 public:
  ...
  static auto Transfer(v8::Local<v8::ArrayBuffer> handle)
...
class ExternalCopySharedArrayBuffer : public ExternalCopyAnyBuffer {
 public:
  explicit ExternalCopySharedArrayBuffer(
...
class ExternalCopyArrayBufferView : public ExternalCopy {
 ...
 public:
  ExternalCopyArrayBufferView(std::unique_ptr<ExternalCopyAnyBuffer> buffer,
                              ViewType type,
                              size_t byte_offset,
                              size_t byte_length);
  auto CopyInto(bool transfer_in = false) -> v8::Local<v8::Value> final;
