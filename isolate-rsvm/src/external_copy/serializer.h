...
class SerializerDelegate : public SerializationDelegateBase,
 ...
 public:
  ...
  auto GetSharedArrayBufferId(
      ...
      -> v8::Maybe<uint32_t> final;
  auto GetWasmModuleTransferId(v8::Isolate* isolate,
                               v8::Local<v8::WasmModuleObject> module)
      -> v8::Maybe<uint32_t> final;
  auto WriteHostObject(v8::Isolate* isolate, v8::Local<v8::Object> object)
      -> v8::Maybe<bool> final;
...
class DeserializerDelegate : public SerializationDelegateBase,
                             public v8::ValueDeserializer::Delegate {
 public:
  using SerializationDelegateBase::SerializationDelegateBase;

  void SetDeserializer(v8::ValueDeserializer* deserializer) {
    this->deserializer = deserializer;
  }

  auto GetSharedArrayBufferFromId(v8::Isolate* isolate, uint32_t clone_id)
      -> v8::MaybeLocal<v8::SharedArrayBuffer> final;
  auto GetWasmModuleFromId(v8::Isolate* isolate, uint32_t transfer_id)
      -> v8::MaybeLocal<v8::WasmModuleObject> final;
...
/**
 * Abstract serializer which manages boilerplate and lifecycle management of the
 * serialized blob
 */
 ...
 protected:
  ...
  auto Deserialize(Fn fn) {
    ...
    if (!fn(deserializer, context)) {
      // ValueDeserializer throws an unhelpful message when it fails to allocate
      // an ArrayBuffer, so detect that case here and throw an appropriate
      // message.
