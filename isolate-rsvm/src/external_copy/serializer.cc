 ...
 */
ExternalCopySerialized::ExternalCopySerialized(Local<Value> value,
                                               ArrayRange transfer_list)
    : BaseSerializer{[&](ValueSerializer& serializer, Local<Context> context) {
        // Mark ArrayBuffers as transferred, but don't actually transfer yet
        // otherwise it will invalidate array views before they are transferred
        int ii = 0;
  ...
  Deserialize([&](ValueDeserializer& deserializer, Local<Context> context) {
    ...
    for (unsigned ii = 0; ii < array_buffers.size(); ++ii) {
      deserializer.TransferArrayBuffer(
          ii,
          array_buffers[ii]->CopyIntoCheckHeap(transfer_in).As<ArrayBuffer>());
    }
