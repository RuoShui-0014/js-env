...
struct ExternalCopyBigInt : public ExternalCopy {
 public:
  ...
  auto CopyInto(bool /*transfer_in*/ = false) -> Local<Value> final {
    return Unmaybe(
        BigInt::NewFromWords(Isolate::GetCurrent()->GetCurrentContext(),
                             sign_bit, words.size(), words.data()));
  }
...
ExternalCopy::ExternalCopy(ExternalCopy&& that) noexcept
...
auto ExternalCopy::Copy(Local<Value> value,
                        bool transfer_out,
  ...
  if (copy) {
    ...
    if (!transfer_out) {
      transfer_out = std::find(transfer_list.begin(), transfer_list.end(),
                               array_buffer) != transfer_list.end();
    }
    if (transfer_out) {
    ...
    return std::make_unique<ExternalCopySharedArrayBuffer>(
    ...
    // Sometimes TypedArrays don't actually have a real buffer allocated for
    // them. The call to `Buffer()` below will force v8 to attempt to create a
    // buffer if it doesn't exist, and if there is an allocation failure it will
    // crash the process.
    if (!view->HasBuffer()) {
    ...
    // `Buffer()` returns a Local<ArrayBuffer> but it may be a
    ...
    if (tmp->IsArrayBuffer()) {
      ...
      // Grab byte_offset and byte_length before the transfer because
      // "neutering" the array buffer will null these out
      ...
      if (transfer_out) {
        ...
        external_buffer =
            std::make_unique<ExternalCopyArrayBuffer>(array_buffer);
      }
      return std::make_unique<ExternalCopyArrayBufferView>(
          std::move(external_buffer), type, byte_offset, byte_length);
    } else {
      assert(tmp->IsSharedArrayBuffer());
      Local<SharedArrayBuffer> array_buffer = tmp.As<SharedArrayBuffer>();
      return std::make_unique<ExternalCopyArrayBufferView>(
          std::make_unique<ExternalCopySharedArrayBuffer>(array_buffer), type,
          view->ByteOffset(), view->ByteLength());
    }
  } else if (value->IsObject()) {
    return std::make_unique<ExternalCopySerialized>(value.As<Object>(),
                                                    transfer_list);
  } else {
...
auto ExternalCopy::CopyIfPrimitive(Local<Value> value)
    -> std::unique_ptr<ExternalCopy> {
  if (!value->IsObject()) {
...
auto ExternalCopy::CopyThrownValue(Local<Value> value)
    -> std::unique_ptr<ExternalCopy> {
  if (value->IsObject()) {
    ...
    std::string name =
        *String::Utf8Value{isolate, object->GetConstructorName()};
    ...
    if (error_type != ExternalCopyError::ErrorType::CustomError ||
      ...
      return std::make_unique<ExternalCopyError>(
          error_type, std::move(name_copy), std::move(message_copy),
          std::move(stack_copy));
    }
  }
  ...
  return std::make_unique<ExternalCopyError>(
      ...
      "An object was thrown from supplied code within isolated-vm, but that "
      "object was not an instance of `Error`.");
}
 ...
 */
ExternalCopyError::ExternalCopyError(ErrorType error_type,
...
auto ExternalCopyError::CopyInto(bool /*transfer_in*/) -> Local<Value> {
  ...
  switch (error_type) {
    default:
      ...
      if (name) {
        Unmaybe(handle.As<Object>()->DefineOwnProperty(
            context, StringTable::Get().name, name.CopyInto(),
  ...
  if (this->stack) {
    ...
    Unmaybe(Local<Object>::Cast(handle)->Set(context, StringTable::Get().stack,
                                             stack));
  }
  return handle;
}
 ...
 */
ExternalCopyArrayBuffer::ExternalCopyArrayBuffer(const void* data,
                                                 size_t length)
    : ExternalCopyAnyBuffer{ArrayBuffer::NewBackingStore(
          std::malloc(length),
          length,
          [](void* data, size_t /*length*/, void* /*param*/) {
            std::free(data);
          },
          nullptr)} {
...
ExternalCopyArrayBuffer::ExternalCopyArrayBuffer(Local<ArrayBuffer> handle)
    : ExternalCopyArrayBuffer{handle->GetBackingStore()->Data(),
                              handle->ByteLength()} {}

auto ExternalCopyArrayBuffer::Transfer(Local<ArrayBuffer> handle)
    -> std::unique_ptr<ExternalCopyArrayBuffer> {
...
auto ExternalCopyArrayBuffer::CopyInto(bool transfer_in) -> Local<Value> {
  if (transfer_in) {
    ...
    auto handle =
        ArrayBuffer::New(Isolate::GetCurrent(), std::move(backing_store));
    auto* allocator = IsolateEnvironment::GetCurrent().GetLimitedAllocator();
    ...
    if (allocator != nullptr && !allocator->Check(size)) {
      // ArrayBuffer::New will crash the process if there is an allocation
      // failure, so we check here.
      throw RuntimeRangeError("Array buffer allocation failed");
    }
 ...
 */
ExternalCopySharedArrayBuffer::ExternalCopySharedArrayBuffer(
    Local<SharedArrayBuffer> handle)
    : ExternalCopyAnyBuffer{handle->GetBackingStore()} {}

auto ExternalCopySharedArrayBuffer::CopyInto(bool /*transfer_in*/)
    -> Local<Value> {
  auto backing_store = *this->backing_store.read();
  size_t size = backing_store->ByteLength();
  auto handle =
      SharedArrayBuffer::New(Isolate::GetCurrent(), std::move(backing_store));
  auto* allocator = IsolateEnvironment::GetCurrent().GetLimitedAllocator();
  if (allocator != nullptr) {
    allocator->Track(handle, size);
  }
...
template <typename T>
auto NewTypedArrayView(Local<T> buffer,
                       ExternalCopyArrayBufferView::ViewType type,
                       size_t byte_offset,
                       size_t byte_length) -> Local<Value> {
  switch (type) {
...
auto ExternalCopyArrayBufferView::CopyInto(bool transfer_in) -> Local<Value> {
  ...
  if (buffer->IsArrayBuffer()) {
    return NewTypedArrayView(buffer.As<ArrayBuffer>(), type, byte_offset,
                             byte_length);
  } else {
    return NewTypedArrayView(buffer.As<SharedArrayBuffer>(), type, byte_offset,
                             byte_length);
  }
}
