...
/**
 * This file is compiled *without* runtime type information, which matches the
 * nodejs binary and allows the serializer delegates to resolve correctly.
 */
...
auto SerializerDelegate::GetSharedArrayBufferId(
    Isolate* /*isolate*/,
    Local<SharedArrayBuffer> shared_array_buffer) -> Maybe<uint32_t> {
  auto result = Nothing<uint32_t>();
  detail::RunBarrier([&]() {
    transferables.emplace_back(
        std::make_unique<ExternalCopySharedArrayBuffer>(shared_array_buffer));
    result = Just<uint32_t>(transferables.size() - 1);
  });
  return result;
}

auto SerializerDelegate::GetWasmModuleTransferId(Isolate* /*isolate*/,
...
auto SerializerDelegate::WriteHostObject(Isolate* /*isolate*/,
                                         Local<Object> object) -> Maybe<bool> {
...
auto DeserializerDelegate::ReadHostObject(Isolate* /*isolate*/)
    -> MaybeLocal<Object> {
...
auto DeserializerDelegate::GetSharedArrayBufferFromId(Isolate* /*isolate*/,
                                                      uint32_t clone_id)
...
auto DeserializerDelegate::GetWasmModuleFromId(Isolate* isolate,
                                               uint32_t transfer_id)
    -> MaybeLocal<WasmModuleObject> {
  MaybeLocal<WasmModuleObject> result;
  detail::RunBarrier([&]() {
    result = WasmModuleObject::FromCompiledModule(isolate,
                                                  wasm_modules[transfer_id]);
  });
  return result;
}
