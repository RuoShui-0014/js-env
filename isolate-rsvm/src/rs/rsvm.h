 ...
 public:
  void Operation(Local<Object> handle, Local<Value> result) {
    if (log) {
      ...
      Local<Value> rsvm = context->Global()
                              ->Get(context, v8_str(isolate, "rsvm"))
                              .ToLocalChecked();
      if (rsvm->IsObject()) {
        Local<Value> logFunction = rsvm.As<Object>()
                                       ->Get(context, v8_str("logFunction"))
                                       .ToLocalChecked();
        if (logFunction->IsObject()) {
          Local<Value> callback = logFunction.As<Object>()
          ...
          if (callback->IsFunction()) {
            ...
            Local<Value> params[]{
                ...
                paramsArray, result};
            callback.As<Function>()->Call(context, Null(isolate),
                                          sizeof(params) / sizeof(Local<Value>),
                                          params);
          }
        }
      }
    ...
    if (glog) {
      ...
      if (rsvm->IsObject()) {
        Local<Value> logFunction = rsvm.As<Object>()
                                       ...
                                       .ToLocalChecked();
        if (logFunction->IsObject()) {
          Local<Value> windowGetter = logFunction.As<Object>()
                                          ->Get(context, v8_str("windowGetter"))
                                          .ToLocalChecked();
          if (windowGetter->IsFunction()) {
            Local<Value> params[]{
                v8_str("window"), property,
                global->Get(context, property).ToLocalChecked()};
            windowGetter.As<Function>()->Call(
                context, info.Holder(), sizeof(params) / sizeof(Local<Value>),
                params);
          }
        }
      }
    ...
    if (glog) {
      ...
      if (rsvm->IsObject()) {
        Local<Value> logFunction = rsvm.As<Object>()
                                       ...
                                       .ToLocalChecked();
        if (logFunction->IsObject()) {
          Local<Value> windowSetter = logFunction.As<Object>()
                                          ->Get(context, v8_str("windowSetter"))
                                          .ToLocalChecked();
          if (windowSetter->IsFunction()) {
            Local<Value> params[]{
                v8_str("window"), property, value,
                global->Get(context, property).ToLocalChecked()};
            windowSetter.As<Function>()->Call(
                context, info.Holder(), sizeof(params) / sizeof(Local<Value>),
                params);
          }
        }
...
class InterceptHook {
 public:
  ...
  ~InterceptHook() { window->glog = glog; }
