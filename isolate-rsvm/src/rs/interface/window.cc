...
void GlobalNameGetter(Local<Name> property,
...
void GlobalNameSetter(Local<Name> property,
                      Local<Value> value,
                      const PropertyCallbackInfo<Value>& info) {
  Isolate* isolate = info.GetIsolate();
...
void GlobalIndexGetter(uint32_t index,
                       const PropertyCallbackInfo<Value>& info) {
  Isolate* isolate = info.GetIsolate();
...
void GlobalIndexSetter(uint32_t index,
                       Local<Value> value,
                       const PropertyCallbackInfo<Value>& info) {
  Isolate* isolate = info.GetIsolate();
...
void WindowConstructorCallback(const FunctionCallbackInfo<Value>& info) {
  info.GetIsolate()->ThrowException(
      Exception::TypeError(ExceptionMessages::ConstructorNew()));
}
...
Local<FunctionTemplate> InterfaceWindowTemplate(Isolate* isolate) {
  ...
  v8_set_property(prototype_template, Symbol::GetToStringTag(isolate),
                  v8_str(isolate, "Window"),
                  PropertyAttribute::ReadOnly | PropertyAttribute::DontEnum);
  ConstantValueItem constantValueTable[] = {
      {"PERSISTENT", 1,
       static_cast<PropertyAttribute>(PropertyAttribute::ReadOnly |
                                      PropertyAttribute::DontDelete)},
      {"TEMPORARY", 0,
       static_cast<PropertyAttribute>(PropertyAttribute::ReadOnly |
                                      PropertyAttribute::DontDelete)},
  };
  v8_defineConstantValues(isolate, prototype_template,
                          CONFIGS(constantValueTable));

  Local<ObjectTemplate> instance_template =
      interface_template->InstanceTemplate();
  instance_template->SetInternalFieldCount(1);
  v8_set_property(instance_template, v8_str(isolate, "Window"),
                  interface_template, PropertyAttribute::DontEnum);

  Local<Signature> signature = Signature::New(isolate, interface_template);
  AttributeItem attributeTable[] = {
      {"window", WindowGetCallback, nullptr, PropertyAttribute::DontDelete,
       signature},
  };
...
Local<ObjectTemplate> CreateRsGlobalTemplate(Isolate* isolate, bool intercept) {
  ...
  v8_set_property(instance_template, v8_str(isolate, "rsvm"),
                  CreateRSVM(isolate, intercept),
                  PropertyAttribute::DontDelete | PropertyAttribute::DontEnum);

  if (intercept) {
    NamedPropertyHandlerConfiguration nameHandler(
        GlobalNameGetter, GlobalNameSetter, nullptr, nullptr, nullptr, nullptr,
        nullptr);
    instance_template->SetHandler(nameHandler);

    IndexedPropertyHandlerConfiguration indexHandler(
        GlobalIndexGetter, GlobalIndexSetter, nullptr, nullptr, nullptr,
        nullptr, nullptr);
