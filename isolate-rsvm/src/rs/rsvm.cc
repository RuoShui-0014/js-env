...
void NameGetter(Local<Name> property, const PropertyCallbackInfo<Value>& info) {
  ...
  Local<Function> callback =
      handler->Get(context, v8_str("getter")).ToLocalChecked().As<Function>();

  Local<Value> params[]{info.This(), property};
  MaybeLocal<Value> maybe_result = callback->Call(
      context, info.This(), sizeof(params) / sizeof(Local<Value>), params);
  Local<Value> result;
...
void NameSetter(Local<Name> property,
                Local<Value> value,
  ...
  Local<Object> handler = info.Data().As<Object>();
  Local<Function> callback =
      handler->Get(context, v8_str("setter")).ToLocalChecked().As<Function>();
...
void NameQuery(Local<Name> property,
  ...
  Local<Object> handler = info.Data().As<Object>();
  Local<Function> callback =
      handler->Get(context, v8_str("query")).ToLocalChecked().As<Function>();
...
void NameDefiner(Local<Name> property,
  ...
  Local<Object> handler = info.Data().As<Object>();
  Local<Function> callback =
      handler->Get(context, v8_str("definer")).ToLocalChecked().As<Function>();
  ...
  if (desc.has_writable()) {
    obj->Set(context, v8_str("writable"),
             Boolean::New(isolate, desc.writable()));
  }
  if (desc.has_configurable()) {
    obj->Set(context, v8_str("configurable"),
             Boolean::New(isolate, desc.configurable()));
  }
  if (desc.has_enumerable()) {
    obj->Set(context, v8_str("enumerable"),
             Boolean::New(isolate, desc.enumerable()));
  }
...
void NameDeleter(Local<Name> property,
  ...
  Local<Object> handler = info.Data().As<Object>();
  Local<Function> callback =
      handler->Get(context, v8_str("deleter")).ToLocalChecked().As<Function>();
...
void NameDescriptor(Local<Name> property,
                    const PropertyCallbackInfo<Value>& info) {
  Isolate* isolate = info.GetIsolate();
  Local<Context> context = isolate->GetCurrentContext();
  ...
  Local<Object> handler = info.Data().As<Object>();
  Local<Function> callback = handler->Get(context, v8_str("descriptor"))
                                 .ToLocalChecked()
                                 .As<Function>();
...
void NameEnumerator(const PropertyCallbackInfo<Array>& info) {
  ...
  Local<Object> handler = info.Data().As<Object>();
  Local<Function> callback = handler->Get(context, v8_str("enumerator"))
                                 .ToLocalChecked()
                                 .As<Function>();
...
void RsCreateNameInterceptor(const FunctionCallbackInfo<Value>& info) {
  ...
  isolate->ThrowException(Exception::TypeError(
      v8_str("arguments number is not 1 or arguments[0] type is not object")));
}
void RsCreateIndexInterceptor(const FunctionCallbackInfo<Value>& info) {
  ...
  if (info.Length() == 1 && info[0]->IsObject()) {
    ...
    IndexedPropertyHandlerConfiguration indexHandler(
        (IndexedPropertyGetterCallback) nullptr, nullptr, nullptr, nullptr,
        nullptr, nullptr, nullptr, info[0], PropertyHandlerFlags::kNone);
    if (handle->HasOwnProperty(context, v8_str("getter")).ToChecked()) {
      indexHandler.getter = IndexGetter;
    }
    if (handle->HasOwnProperty(context, v8_str("setter")).ToChecked()) {
      indexHandler.setter = IndexSetter;
    }
    if (handle->HasOwnProperty(context, v8_str("query")).ToChecked()) {
      indexHandler.query = IndexQuery;
    }
    if (handle->HasOwnProperty(context, v8_str("definer")).ToChecked()) {
      indexHandler.definer = IndexDefiner;
    }
    if (handle->HasOwnProperty(context, v8_str("deleter")).ToChecked()) {
      indexHandler.deleter = IndexDeleter;
    }
    if (handle->HasOwnProperty(context, v8_str("descriptor")).ToChecked()) {
      indexHandler.descriptor = IndexDescriptor;
    }
    if (handle->HasOwnProperty(context, v8_str("enumerator")).ToChecked()) {
      indexHandler.enumerator = IndexEnumerator;
    }
...
void DocumentAll(const FunctionCallbackInfo<Value>& info) {
  ...
  Local<Function> fun = handle->Get(context, v8_str("callAsFunction"))
                            .ToLocalChecked()
                            .As<Function>();
  ...
  MaybeLocal<Value> result =
      fun->Call(context, info.This(), info.Length(), params);
...
void RsCreateDocumentAll(const FunctionCallbackInfo<Value>& info) {
  ...
  if (!info[0]->IsObject()) {
    Local<Value> error = Exception::TypeError(
        v8_str("First argument type must be 'object' handle."));
    isolate->ThrowException(error);
    return;
  }
...
void ConstructorErrorCallback(const FunctionCallbackInfo<Value>& info) {
  ...
  MaybeLocal<Value> has = info.This()->GetPrivate(
      context, Private::ForApi(isolate, v8_str("__memory__")));
  if (has.IsEmpty() || has.ToLocalChecked()->IsUndefined()) {
    isolate->ThrowException(
        Exception::TypeError(v8_str("Illegal constructor")));
  }
}
void ConstructorNoErrorCallback(const FunctionCallbackInfo<Value>& info) {
  ...
  if (!info.IsConstructCall()) {
    isolate->ThrowException(
        Exception::TypeError(ExceptionMessages::ConstructorCalledAsFunction()));
    return;
  }
  ...
  Local<Value> callback =
      handle->Get(context, v8_str("callback")).ToLocalChecked();
  Local<Value> params[10]{};
  for (int i = 0; i < info.Length(); i++) {
    params[i] = info[i];
  }
  Local<Value> result = Undefined(isolate).As<Value>();
  MaybeLocal<Value> maybe_result = callback.As<Function>()->Call(
  ...
  if (rsvm->IsObject() && log) {
    Local<Value> logFunction =
        rsvm.As<Object>()->Get(context, v8_str("logFunction")).ToLocalChecked();
    if (logFunction->IsObject()) {
      Local<Value> func = logFunction.As<Object>()
                              ->Get(context, v8_str("constructor"))
                              .ToLocalChecked();
      if (func->IsFunction()) {
        ...
        Local<Value> ps[] = {
            handle->Get(context, v8_str("name")).ToLocalChecked(), paramsAry,
            result};
        func.As<Function>()->Call(context, Null(isolate),
                                  sizeof(ps) / sizeof(Local<Value>), ps);
      }
    }
...
void GetterCallback(const FunctionCallbackInfo<Value>& info) {
  ...
  MaybeLocal<Value> maybe_result =
      callback.As<Function>()->Call(context, info.This(), 0, nullptr);
  Local<Value> result;
  if (maybe_result.ToLocal(&result)) {
    info.GetReturnValue().Set(result);
  }
  ...
  if (info.Length() == 2 && info[0]->IsString() && info[1]->IsFunction()) {
    ...
    Local<FunctionTemplate> fun_temp =
        FunctionTemplate::NewWithCFunctionOverloads(
            isolate, GetterCallback, handle, Local<Signature>(), 0,
            ConstructorBehavior::kThrow);
    fun_temp->SetClassName(
        String::Concat(isolate, v8_str("get "), info[0].As<String>()));
...
void SetterCallback(const FunctionCallbackInfo<Value>& info) {
  ...
  Local<Object> handle = info.Data().As<Object>();
  Local<Function> callback =
      handle->Get(context, v8_str("callback")).ToLocalChecked().As<Function>();
...
void RsCreateSetter(const FunctionCallbackInfo<Value>& info) {
  ...
  if (info.Length() >= 2 && info[0]->IsString() && info[1]->IsFunction()) {
    ...
    Local<FunctionTemplate> fun_temp = FunctionTemplate::New(
        isolate, SetterCallback, handle, Local<Signature>(), 1,
        ConstructorBehavior::kThrow);
    fun_temp->SetClassName(
        String::Concat(isolate, v8_str("set "), info[0].As<String>()));
...
void RsCreateWindowGetter(const FunctionCallbackInfo<Value>& info) {
  ...
  if (info.Length() == 2 && info[0]->IsString() && info[1]->IsFunction()) {
    ...
    Local<FunctionTemplate> fun_temp = FunctionTemplate::New(
        isolate, WindowGetterCallback, handle, Local<Signature>(), 0,
        ConstructorBehavior::kThrow);
    fun_temp->SetClassName(
        String::Concat(isolate, v8_str("get "), info[0].As<String>()));
...
void RsCreateWindowSetter(const FunctionCallbackInfo<Value>& info) {
  ...
  if (info.Length() == 2 && info[0]->IsString() && info[1]->IsFunction()) {
    Local<Object> handle =
        ObjectTemplate::New(isolate)->NewInstance(context).ToLocalChecked();
    handle->Set(context, v8_str("name"), info[0]);
    handle->Set(context, v8_str("length"), Number::New(isolate, 1));
    handle->Set(context, v8_str("callback"), info[1]);

    Local<FunctionTemplate> fun_temp = FunctionTemplate::New(
        isolate, WindowSetterCallback, handle, Local<Signature>(), 1,
        ConstructorBehavior::kThrow);
    fun_temp->SetClassName(
        String::Concat(isolate, v8_str("set "), info[0].As<String>()));
...
void RsCreateFunctioncallback(const FunctionCallbackInfo<Value>& info) {
  ...
  MaybeLocal<Value> maybe_result = fun_callback.As<Function>()->Call(
      context, info.This(), info.Length(), params);
  Local<Value> result;
  if (maybe_result.ToLocal(&result)) {
    if (info.IsConstructCall()) {
...
void RsCreateFunction(const FunctionCallbackInfo<Value>& info) {
  if (info.Length() == 4 && info[0]->IsBoolean() && info[1]->IsString() &&
    ...
    Local<FunctionTemplate> fun_temp = FunctionTemplate::New(
        ...
        info[0]->IsTrue() ? ConstructorBehavior::kAllow
                          : ConstructorBehavior::kThrow);
...
void RsCreateStaticFunction(const FunctionCallbackInfo<Value>& info) {
  if (info.Length() == 3 && info[0]->IsString() && info[1]->IsNumber() &&
    ...
    Local<Function> fun_temp =
        Function::New(context, RsCreateFunctioncallback, info[2], length,
                      ConstructorBehavior::kThrow)
            .ToLocalChecked();
...
void RsSetPrivateProperty(const FunctionCallbackInfo<Value>& info) {
  ...
  if (info.Length() == 3 && info[0]->IsObject() && info[1]->IsString()) {
    Local<Context> context = isolate->GetCurrentContext();
    Maybe<bool> isok = info[0].As<Object>()->SetPrivate(
        context, Private::ForApi(isolate, info[1].As<String>()), info[2]);
    if (isok.ToChecked()) {
  ...
  isolate->ThrowException(Exception::TypeError(
      v8_str("Need 3 args, args[0] is object, args[1] is string!!!")));
}
  ...
  if (info.Length() == 2 && info[0]->IsObject() && info[1]->IsString()) {
    ...
    Local<Value> value =
        info[0]
            .As<Object>()
            ->GetPrivate(context,
                         Private::ForApi(isolate, info[1].As<String>()))
            .ToLocalChecked();
    info.GetReturnValue().Set(value);
    return;
  }
  isolate->ThrowException(Exception::TypeError(
      v8_str("Need 2 args, args[0] is object, args[1] is string!!!")));
}
...
Local<ObjectTemplate> CreateRSVM(Isolate* isolate, bool intercept) {
  ...
  AttributeItem attributeItem[] = {
      {"log", RSWindow::LogGetCallback, RSWindow::LogSetCallback,
  ...
  if (intercept) {
    AttributeItem attributeItem[] = {
        {"glog", RSWindow::GlogGetCallback, RSWindow::GlogSetCallback,
         PropertyAttribute::DontDelete},
    };
    v8_defineAttributes(isolate, instance_template, CONFIGS(attributeItem));
  }
