...
/**
 * isolated-vm could start timers from different threads which libuv isn't
 * really cut out for, so I'm rolling my own here. The goal of the library is to
 * have atomic timers without spawning a new thread for each timer.
 */
struct timer_data_t;
class timer_t {
 public:
  ...
  timer_t(uint32_t ms, const callback_t& callback)
      : timer_t{ms, nullptr, callback} {}
