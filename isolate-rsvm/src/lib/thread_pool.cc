...
void thread_pool_t::exec(affinity_t& affinity, entry_t* entry, void* param) {
  ...
  if (affinity.previous < thread_data.size() &&
      thread_data[affinity.previous].entry == nullptr) {
  ...
  if (thread == std::numeric_limits<unsigned>::max()) {
    if (desired_size > thread_data.size()) {
      // Thread pool hasn't yet reached `desired_size`, so we can make a new
      // thread
...
auto thread_pool_t::new_thread(std::lock_guard<std::mutex>& /*lock*/)
    -> size_t {
