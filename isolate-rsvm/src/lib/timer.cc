...
/**
 * Contains data on a timer. This is shared between the timer_t handle and the
 * thread responsible for the timer.
 */
struct timer_data_t {
  timer_data_t(std::chrono::steady_clock::time_point timeout,
               void** holder,
  ...
  void pause() { paused_at = std::chrono::steady_clock::now(); }
  ...
  struct cmp {
    auto operator()(const std::shared_ptr<timer_data_t>& left,
                    const std::shared_ptr<timer_data_t>& right) const {
...
/**
 * Stash these here in case statics are destroyed while the module is unloading
 * but timers are still active
 */
...
struct timer_thread_t {
  ...
  std::priority_queue<std::shared_ptr<timer_data_t>,
...
timer_t::timer_t(uint32_t ms, void** holder, const callback_t& callback) {
  ...
  data = std::make_shared<timer_data_t>(
      std::chrono::steady_clock::now() + std::chrono::milliseconds{ms}, holder,
      callback, lock);
...
void timer_t::resume(void*& holder) {
  ...
  if (holder != nullptr) {
    ...
    if (data.threadless_self) {
      timer_thread_t::start_or_join_timer(std::move(data.threadless_self),
                                          lock);
    }
  }
}
...
void timer_t::wait_detached(uint32_t ms, const callback_t& callback) {
  std::lock_guard<std::mutex> lock{global_shared_state->mutex};
  timer_thread_t::start_or_join_timer(
      std::make_shared<timer_data_t>(
