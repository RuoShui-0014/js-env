...
class DereferenceHandleTransferable : public Transferable {
 public:
  DereferenceHandleTransferable(shared_ptr<IsolateHolder> isolate,
...
class DereferenceHandle : public TransferableHandle {
 public:
  DereferenceHandle(shared_ptr<IsolateHolder> isolate,
                    RemoteHandle<v8::Value> reference)
      : isolate{std::move(isolate)}, reference{std::move(reference)} {}
  ...
  auto TransferOut() -> std::unique_ptr<Transferable> final {
    ...
    return std::make_unique<DereferenceHandleTransferable>(
        std::move(isolate), std::move(reference));
  }
...
ReferenceData::ReferenceData(Local<Value> value, bool inherit)
    : ReferenceData{
...
ReferenceData::ReferenceData(shared_ptr<IsolateHolder> isolate,
...
auto ReferenceHandle::Definition() -> Local<FunctionTemplate> {
  return Inherit<TransferableHandle>(MakeClass(
      "Reference", ConstructorFunction<decltype(&New), &New>{}, "deref",
      MemberFunction<decltype(&ReferenceHandle::Deref),
                     &ReferenceHandle::Deref>{},
      "derefInto",
      MemberFunction<decltype(&ReferenceHandle::DerefInto),
                     &ReferenceHandle::DerefInto>{},
      "release",
      MemberFunction<decltype(&ReferenceHandle::Release),
                     &ReferenceHandle::Release>{},
      "copy",
      MemberFunction<decltype(&ReferenceHandle::Copy<1>),
                     &ReferenceHandle::Copy<1>>{},
      "copySync",
      MemberFunction<decltype(&ReferenceHandle::Copy<0>),
                     &ReferenceHandle::Copy<0>>{},
      "delete",
      MemberFunction<decltype(&ReferenceHandle::Delete<1>),
                     &ReferenceHandle::Delete<1>>{},
      "deleteIgnored",
      MemberFunction<decltype(&ReferenceHandle::Delete<2>),
                     &ReferenceHandle::Delete<2>>{},
      "deleteSync",
      MemberFunction<decltype(&ReferenceHandle::Delete<0>),
                     &ReferenceHandle::Delete<0>>{},
      "get",
      MemberFunction<decltype(&ReferenceHandle::Get<1>),
                     &ReferenceHandle::Get<1>>{},
      "getSync",
      MemberFunction<decltype(&ReferenceHandle::Get<0>),
                     &ReferenceHandle::Get<0>>{},
      "set",
      MemberFunction<decltype(&ReferenceHandle::Set<1>),
                     &ReferenceHandle::Set<1>>{},
      "setIgnored",
      MemberFunction<decltype(&ReferenceHandle::Set<2>),
                     &ReferenceHandle::Set<2>>{},
      "setSync",
      MemberFunction<decltype(&ReferenceHandle::Set<0>),
                     &ReferenceHandle::Set<0>>{},
      "apply",
      MemberFunction<decltype(&ReferenceHandle::Apply<1>),
                     &ReferenceHandle::Apply<1>>{},
      "applyIgnored",
      MemberFunction<decltype(&ReferenceHandle::Apply<2>),
                     &ReferenceHandle::Apply<2>>{},
      "applySync",
      MemberFunction<decltype(&ReferenceHandle::Apply<0>),
                     &ReferenceHandle::Apply<0>>{},
      "applySyncPromise",
      MemberFunction<decltype(&ReferenceHandle::Apply<4>),
                     &ReferenceHandle::Apply<4>>{},
      "typeof",
      MemberAccessor<decltype(&ReferenceHandle::TypeOfGetter),
                     &ReferenceHandle::TypeOfGetter>{}));
}

auto ReferenceHandle::New(Local<Value> value, MaybeLocal<Object> options)
    -> unique_ptr<ReferenceHandle> {
  auto inherit =
      ReadOption<bool>(options, StringTable::Get().unsafeInherit, false);
...
auto ReferenceHandle::Deref(MaybeLocal<Object> maybe_options) -> Local<Value> {
  ...
  bool release =
      ReadOption<bool>(maybe_options, StringTable::Get().release, false);
...
/**
 * Return a handle which will dereference itself when passing into another
 * isolate.
 */
auto ReferenceHandle::DerefInto(MaybeLocal<Object> maybe_options)
    -> Local<Value> {
  CheckDisposed();
  bool release =
      ReadOption<bool>(maybe_options, StringTable::Get().release, false);
  Local<Value> ret =
      ClassHandle::NewInstance<DereferenceHandle>(isolate, reference);
  if (release) {
    Release();
  }
  return ret;
...
class ApplyRunner : public ThreePhaseTask {
 public:
  ApplyRunner(ReferenceHandle& that,
    ...
    if (maybe_options.ToLocal(&options)) {
      ...
      arguments_transfer_options =
          TransferOptions{ReadOption<MaybeLocal<Object>>(
              options, StringTable::Get().arguments, {})};
      return_transfer_options =
          TransferOptions{ReadOption<MaybeLocal<Object>>(
                              options, StringTable::Get().result, {}),
  ...
  void Phase2() final {
    ...
    Local<Value> result = RunWithTimeout(
        timeout, [&fn, &context_handle, &recv_inner, &argv_inner]() {
          return fn.As<Function>()->Call(
              context_handle, recv_inner, argv_inner.size(),
              argv_inner.empty() ? nullptr : &argv_inner[0]);
        });
  ...
  auto Phase2Async(Scheduler::AsyncWait& wait) -> bool final {
    ...
    if (!(return_transfer_options ==
          TransferOptions{TransferOptions::Type::Reference})) {
    ...
    if (value->IsPromise()) {
      ...
      // This is only called from the default isolate, so we don't need an
      // IsolateSpecific
      static Persistent<Function> callback_persistent{isolate,
                                                      CompileAsyncWrapper()};
 ...
 private:
  /**
   * This is an internal callback that will be called after a Promise returned
   * from `applySyncPromise` has resolved
   */
  static void AsyncCallback(const v8::FunctionCallbackInfo<v8::Value>& info) {
    // It's possible the invocation timed out, in which case the ApplyRunner
    // will be dead. The shared_ptr<bool> here will be marked as true and we can
    // exit early.
    auto* did_finish_ptr =
        reinterpret_cast<shared_ptr<char>*>(info[1].As<External>()->Value());
    ...
    ApplyRunner& self =
        *reinterpret_cast<ApplyRunner*>(info[0].As<External>()->Value());
    if (info.Length() == 3) {
      // Resolved
      FunctorRunners::RunCatchExternal(
          IsolateEnvironment::GetCurrent().DefaultContext(),
          [&self, &info]() { self.ret = TransferOut(info[2]); },
  ...
  /**
   * The C++ promise interface is a little clumsy so this does some work in JS
   * for us. This function is called once and returns a JS function that will be
   * reused.
   */
  static auto CompileAsyncWrapper() -> Local<Function> {
    Isolate* isolate = Isolate::GetCurrent();
    Local<Context> context = IsolateEnvironment::GetCurrent().DefaultContext();
    Local<Script> script = Unmaybe(Script::Compile(
        context, v8_string("'use strict';"
    ...
    Local<Value> callback_fn = Unmaybe(
        FunctionTemplate::New(isolate, AsyncCallback)->GetFunction(context));
    Local<Value> inner_fn = Unmaybe(outer_fn.As<Function>()->Call(
        context, Undefined(isolate), 1, &callback_fn));
  ...
  shared_ptr<char>
      did_finish;  // GCC 5.4.0 `std::make_shared<bool>(...)` is broken(?)
...
template <int async>
auto ReferenceHandle::Apply(MaybeLocal<Value> recv_handle,
                            Maybe<ArrayRange> maybe_arguments,
                            MaybeLocal<Object> maybe_options) -> Local<Value> {
  return ThreePhaseTask::Run<async, ApplyRunner>(
      *isolate, *this, recv_handle, maybe_arguments, maybe_options);
}
...
class CopyRunner : public ThreePhaseTask {
 public:
  CopyRunner(const ReferenceHandle& that,
             RemoteHandle<Context> context,
             RemoteHandle<Value> reference)
  ...
  auto Phase3() -> Local<Value> final { return copy->TransferIn(); }
...
auto ReferenceHandle::Copy() -> Local<Value> {
  return ThreePhaseTask::Run<async, CopyRunner>(*isolate, *this, context,
                                                reference);
}
 ...
 protected:
  ...
  auto GetKey(Local<Context> context) -> Local<Name> {
    ...
    return (key_inner->IsString() || key_inner->IsSymbol())
               ? key_inner.As<Name>()
               : Unmaybe(key_inner->ToString(context)).As<Name>();
  }
...
class GetRunner final : public AccessorRunner {
 public:
  GetRunner(ReferenceHandle& target,
            Local<Value> key_handle,
            MaybeLocal<Object> maybe_options)
      : AccessorRunner{target, key_handle},
        options{maybe_options, target.inherit
                                   ? TransferOptions::Type::DeepReference
                                   : TransferOptions::Type::Reference},
        accessors{
            target.accessors ||
            ReadOption(maybe_options, StringTable::Get().accessors, false)},
  ...
  void Phase2() final {
    ...
    ret = TransferOut(
        [&]() {
          if (inherit) {
            // To avoid accessors I guess we have to walk the prototype chain
            // ourselves
            auto target = object;
            if (!accessors) {
              do {
                if (Unmaybe(target->HasOwnProperty(context, name))) {
                  if (Unmaybe(target->HasRealNamedCallbackProperty(context,
                                                                   name))) {
            ...
            return Undefined(isolate).As<Value>();
          } else if (!accessors && Unmaybe(object->HasRealNamedCallbackProperty(
                                       context, name))) {
            throw RuntimeTypeError("Property is getter");
          }
  ...
  auto Phase3() -> Local<Value> final { return ret->TransferIn(); }
...
template <int async>
auto ReferenceHandle::Get(Local<Value> key_handle,
                          MaybeLocal<Object> maybe_options) -> Local<Value> {
  return ThreePhaseTask::Run<async, GetRunner>(*isolate, *this, key_handle,
                                               maybe_options);
}
...
class SetRunner final : public AccessorRunner {
 public:
  SetRunner(ReferenceHandle& that,
            Local<Value> key_handle,
  ...
  void Phase2() final {
    ...
    if (!Unmaybe(
            object->CreateDataProperty(context, GetKey(context), val_inner))) {
...
template <int async>
auto ReferenceHandle::Set(Local<Value> key_handle,
                          Local<Value> val_handle,
                          MaybeLocal<Object> maybe_options) -> Local<Value> {
  return ThreePhaseTask::Run<async, SetRunner>(*isolate, *this, key_handle,
                                               val_handle, maybe_options);
}
