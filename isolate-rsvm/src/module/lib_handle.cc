...
auto LibHandle::Definition() -> Local<FunctionTemplate> {
  return Inherit<TransferableHandle>(MakeClass(
      "Lib", nullptr, "hrtime",
      MemberFunction<decltype(&LibHandle::Hrtime), &LibHandle::Hrtime>{},
      "privateSymbol",
      MemberFunction<decltype(&LibHandle::PrivateSymbol),
                     &LibHandle::PrivateSymbol>{},
      "testHang",
      MemberFunction<decltype(&LibHandle::TestHang), &LibHandle::TestHang>{},
      "testOOM",
      MemberFunction<decltype(&LibHandle::TestOOM), &LibHandle::TestOOM>{}));
}
...
auto LibHandle::Hrtime(MaybeLocal<Array> maybe_diff) -> Local<Value> {
  ...
  if (maybe_diff.ToLocal(&diff)) {
    ...
    uint64_t time_diff =
        Unmaybe(diff->Get(context, 0)).As<Uint32>()->Value() * kNanos +
        Unmaybe(diff->Get(context, 1)).As<Uint32>()->Value();
    time -= time_diff;
  }
  Local<Array> ret = Array::New(isolate, 2);
  Unmaybe(
      ret->Set(context, 0, Uint32::New(isolate, (uint32_t)(time / kNanos))));
  Unmaybe(ret->Set(
      context, 1,
      Uint32::New(isolate, (uint32_t)(time - (time / kNanos) * kNanos))));
  return ret;
}
