...
auto LookupModuleInfo(Local<Module> module) {
  ...
  auto it = std::find_if(range.first, range.second,
                         [&](decltype(*module_map.begin()) data) {
...
ModuleInfo::ModuleInfo(Local<Module> handle)
    : identity_hash{handle->GetIdentityHash()}, handle{handle} {
  ...
  for (int ii = 0; ii < requests.Length(); ii++) {
    ...
    dependency_specifiers.emplace_back(
        *String::Utf8Value{isolate, request->GetSpecifier()});
  }
}
...
ModuleHandle::ModuleHandleTransferable::ModuleHandleTransferable(
...
ModuleHandle::ModuleHandle(shared_ptr<ModuleInfo> info)
    : info(std::move(info)) {}

auto ModuleHandle::Definition() -> Local<FunctionTemplate> {
  return Inherit<TransferableHandle>(
      MakeClass("Module", nullptr, "dependencySpecifiers",
                MemberAccessor<decltype(&ModuleHandle::GetDependencySpecifiers),
                               &ModuleHandle::GetDependencySpecifiers>{},
                "instantiate",
                MemberFunction<decltype(&ModuleHandle::Instantiate),
                               &ModuleHandle::Instantiate>{},
                "instantiateSync",
                MemberFunction<decltype(&ModuleHandle::InstantiateSync),
                               &ModuleHandle::InstantiateSync>{},
                "evaluate",
                MemberFunction<decltype(&ModuleHandle::Evaluate<1>),
                               &ModuleHandle::Evaluate<1>>{},
                "evaluateSync",
                MemberFunction<decltype(&ModuleHandle::Evaluate<0>),
                               &ModuleHandle::Evaluate<0>>{},
                "namespace",
                MemberAccessor<decltype(&ModuleHandle::GetNamespace),
                               &ModuleHandle::GetNamespace>{},
                "release",
                MemberFunction<decltype(&ModuleHandle::Release),
...
auto ModuleHandle::GetDependencySpecifiers() -> Local<Value> {
  ...
  for (size_t ii = 0; ii < length; ++ii) {
    Unmaybe(deps->Set(isolate->GetCurrentContext(), ii,
                      v8_string(info->dependency_specifiers[ii].c_str())));
  }
  return deps;
}
...
void ModuleHandle::InitializeImportMeta(Local<Context> context,
                                        Local<Module> module,
                                        Local<Object> meta) {
  ...
  if (found != nullptr) {
    if (found->meta_callback) {
      detail::RunBarrier([&]() {
        Local<Value> argv[1];
        argv[0] = meta;
        Unmaybe(found->meta_callback.Deref()->Call(
            context, Undefined(context->GetIsolate()), 1, argv));
      });
    }
  }
}
...
/**
 * Implements the module linking logic used by `instantiate`. This is
 * implemented as a class handle so v8 can manage the lifetime of the linker. If
 * a promise fails to resolve then v8 will be responsible for calling the
 * destructor.
 */
class ModuleLinker : public ClassHandle {
 public:
  /**
   * These methods are split out from the main class so I don't have to recreate
   * the class inheritance in v8
   */
  struct Implementation {
    RemoteHandle<Object> linker;
    explicit Implementation(Local<Object> linker) : linker(linker) {}
    virtual ~Implementation() = default;
    virtual void HandleCallbackReturn(ModuleHandle* module,
                                      size_t ii,
                                      Local<Value> value) = 0;
    virtual auto Begin(ModuleHandle& module, RemoteHandle<Context> context)
        -> Local<Value> = 0;
 ...
 public:
  ...
  auto Begin(ModuleHandle& module, RemoteHandle<Context> context)
      -> Local<Value> {
    return impl->Begin(module, std::move(context));
  }

  void ResolveDependency(size_t ii,
                         ModuleInfo& module,
                         ModuleHandle* dependency) {
    {
      // I don't think the lock is actually needed here because this linker has
      // already claimed the whole module, and this code will only be running in
      // a single thread.. but putting up the lock is probably good practice or
      // something.
      std::lock_guard<std::mutex> lock(module.mutex);
      module.resolutions[module.dependency_specifiers[ii]] =
          dependency->GetInfo();
    }
    Link(dependency);
  }
  ...
  void Link(ModuleHandle* module) {
    ...
    for (size_t ii = 0; ii < info->dependency_specifiers.size(); ++ii) {
      ...
      impl->HandleCallbackReturn(module, ii,
                                 Unmaybe(fn->Call(context, recv, 2, argv)));
    }
  }

  void Reset(ModuleInfo::LinkStatus status = ModuleInfo::LinkStatus::None) {
    // Clears out dependency info. If the module wasn't instantiated this resets
    // them back to their original state. If it was instantiated then we don't
    // need the dependencies anymore anyway.
...
/**
 * Runner for `instantiate`. By the time this is invoked the module will already
 * have all its dependencies resolved by the linker.
 */
struct InstantiateRunner : public ThreePhaseTask {
  ...
  static auto ResolveCallback(Local<Context> /*context*/,
    ...
    detail::RunBarrier([&]() {
      ...
      if (found != nullptr) {
        ...
        auto it = resolutions.find(
            *String::Utf8Value{Isolate::GetCurrent(), specifier});
      ...
      throw RuntimeGenericError(
          "Dependency was left unresolved. Please report this error on "
          "github.");
    });
    return ret;
  }

  InstantiateRunner(RemoteHandle<Context> context,
                    shared_ptr<ModuleInfo> info,
                    Local<Object> linker)
      : context(std::move(context)), info(std::move(info)), linker(linker) {
    // Sanity check
    if (this->info->handle.GetIsolateHolder() !=
  ...
  void Phase2() final {
    ...
    // `InstantiateModule` will return Maybe<bool>{true} even when there are
    // exceptions pending. This condition is checked here and a C++ is thrown
    // which will propagate out as a JS exception.
  ...
  auto Phase3() -> Local<Value> final {
    ClassHandle::Unwrap<ModuleLinker>(linker.Deref())
        ->Reset(ModuleInfo::LinkStatus::Linked);
    return Undefined(Isolate::GetCurrent());
  }
};
...
class ModuleLinkerSync : public ModuleLinker::Implementation {
 private:
  void HandleCallbackReturn(ModuleHandle* module,
                            size_t ii,
                            Local<Value> value) final {
    ModuleHandle* resolved =
        value->IsObject()
            ? ClassHandle::Unwrap<ModuleHandle>(value.As<Object>())
            : nullptr;
    if (resolved == nullptr) {
 ...
 public:
  ...
  auto Begin(ModuleHandle& module, RemoteHandle<Context> context)
    ...
    auto info = module.GetInfo();
    return ThreePhaseTask::Run<0, InstantiateRunner>(
        *info->handle.GetIsolateHolder(), context, info, linker.Deref());
  }
};
...
class ModuleLinkerAsync : public ModuleLinker::Implementation {
 private:
  ...
  static auto ModuleResolved(Local<Array> holder, Local<Value> value)
      -> Local<Value> {
    detail::RunBarrier([&]() {
      ModuleHandle* resolved =
          value->IsObject()
      ...
      auto* linker = ClassHandle::Unwrap<ModuleLinker>(
          Unmaybe(holder->Get(context, 0)).As<Object>());
      auto* impl = linker->GetImplementation<ModuleLinkerAsync>();
      if (impl == nullptr) {
        return;
      }
      auto* module = ClassHandle::Unwrap<ModuleHandle>(
          Unmaybe(holder->Get(context, 1)).As<Object>());
  ...
  static auto ModuleRejected(ModuleLinker& linker, Local<Value> error)
      -> Local<Value> {
    detail::RunBarrier([&]() {
      auto* impl = linker.GetImplementation<ModuleLinkerAsync>();
      if (impl != nullptr) {
        Unmaybe(impl->async_handles.Deref<0>()->Reject(
            Isolate::GetCurrent()->GetCurrentContext(), error));
        linker.Reset();
      }
    });
    return Undefined(Isolate::GetCurrent());
  ...
  void HandleCallbackReturn(ModuleHandle* module,
    ...
    promise = Unmaybe(promise->Then(
        context,
        Unmaybe(Function::New(
            context,
            FreeFunctionWithData<decltype(&ModuleResolved), &ModuleResolved>{}
                .callback,
            holder))));
  ...
  void Instantiate() {
    Unmaybe(async_handles.Deref<0>()->Resolve(
        ...
        ThreePhaseTask::Run<1, InstantiateRunner>(
            *info->handle.GetIsolateHolder(), context, info, linker.Deref())));
  }

 public:
  explicit ModuleLinkerAsync(Local<Object> linker)
      : Implementation(linker),
        async_handles(Unmaybe(Promise::Resolver::New(
                          Isolate::GetCurrent()->GetCurrentContext())),
                      Unmaybe(Function::New(
                          Isolate::GetCurrent()->GetCurrentContext(),
                          FreeFunctionWithData<decltype(&ModuleRejected),
                                               &ModuleRejected>{}
                              .callback,
                          linker))) {}
...
auto ModuleHandle::Instantiate(ContextHandle& context_handle,
                               Local<Function> callback) -> Local<Value> {
  auto context = context_handle.GetContext();
  Local<Object> linker_handle =
      ClassHandle::NewInstance<ModuleLinker>(callback);
...
auto ModuleHandle::InstantiateSync(ContextHandle& context_handle,
                                   Local<Function> callback) -> Local<Value> {
  auto context = context_handle.GetContext();
  Local<Object> linker_handle =
      ClassHandle::NewInstance<ModuleLinker>(callback);
...
struct EvaluateRunner : public ThreePhaseTask {
  ...
  EvaluateRunner(shared_ptr<ModuleInfo> info, uint32_t ms)
      : info(std::move(info)), timeout(ms) {}
  ...
  void Phase2() final {
    ...
    result = OptionalTransferOut(RunWithTimeout(
        timeout, [&]() { return mod->Evaluate(context_local); }));
    std::lock_guard<std::mutex> lock(info->mutex);
...
auto ModuleHandle::Evaluate(MaybeLocal<Object> maybe_options) -> Local<Value> {
  auto info = GetInfo();
  int32_t timeout_ms =
      ReadOption<int32_t>(maybe_options, StringTable::Get().timeout, 0);
  return ThreePhaseTask::Run<async, EvaluateRunner>(
      *info->handle.GetIsolateHolder(), info, timeout_ms);
}
...
auto ModuleHandle::GetNamespace() -> Local<Value> {
  ...
  return ClassHandle::NewInstance<ReferenceHandle>(
      info->handle.GetSharedIsolateHolder(), info->global_namespace,
      info->context_handle, ReferenceHandle::TypeOf::Object, true, false);
}
