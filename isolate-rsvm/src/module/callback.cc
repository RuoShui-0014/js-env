...
auto CallbackHandle::Definition() -> Local<FunctionTemplate> {
  return Inherit<TransferableHandle>(
      MakeClass("Callback", ConstructorFunction<decltype(&New), &New>{}));
}

auto CallbackHandle::New(Local<Function> fn, MaybeLocal<Object> options)
    -> std::unique_ptr<CallbackHandle> {
  auto context = Isolate::GetCurrent()->GetCurrentContext();

  // Get function parameter count
  auto maybe_length =
      Unmaybe(fn->Get(context, StringTable::Get().length))->Int32Value(context);
  int length;
  ...
  return std::make_unique<CallbackHandle>(CallbackHandle::Data{
      HandleCast<std::string>(fn->GetDebugName()), length,
      RemoteHandle<Function>{fn}, RemoteHandle<Context>{context}, apply});
}
...
class InvokeRunner : public ThreePhaseTask {
 public:
  InvokeRunner(CallbackHandle::Data data,
  ...
  void Phase2() final {
    ...
    auto maybe_value = fn->Call(context, Undefined(isolate), argv_inner.size(),
 ...
 */
CallbackTransferable::CallbackTransferable(CallbackHandle::Data& data)
    : data{data} {}
...
CallbackTransferable::CallbackTransferable(Local<Function> fn,
    ...
    : data{
          ...
          RemoteHandle<Function>{fn}, RemoteHandle<Context>{context},
...
auto CallbackTransferable::TransferIn() -> Local<Value> {
  ...
  auto fn = Unmaybe(Function::New(context, Invoke, external, data.length,
...
void CallbackTransferable::Invoke(const FunctionCallbackInfo<Value>& info) {
  FunctorRunners::RunCallback(info, [&]() {
    auto& data = *static_cast<CallbackHandle::Data*>(
