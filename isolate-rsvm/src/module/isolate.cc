...
/**
 * The whole library is transferable so you can Inception the library into your
 * isolates.
 */
class LibraryHandle : public TransferableHandle {
 private:
  class LibraryHandleTransferable : public Transferable {
   public:
    auto TransferIn() -> Local<Value> final { return LibraryHandle::Get(); }
  };
 ...
 public:
  ...
  static auto Definition() -> Local<FunctionTemplate> {
    return Inherit<TransferableHandle>(MakeClass(
        "isolated_vm", nullptr, "Callback",
        ClassHandle::GetFunctionTemplate<CallbackHandle>(), "Context",
        ClassHandle::GetFunctionTemplate<ContextHandle>(), "ExternalCopy",
        ClassHandle::GetFunctionTemplate<ExternalCopyHandle>(), "Isolate",
        ClassHandle::GetFunctionTemplate<IsolateHandle>(), "NativeModule",
        ClassHandle::GetFunctionTemplate<NativeModuleHandle>(), "Reference",
        ClassHandle::GetFunctionTemplate<ReferenceHandle>(), "Script",
        ClassHandle::GetFunctionTemplate<ScriptHandle>()));
  }
  ...
  static auto Get() -> Local<Object> {
    Local<Object> library =
        ClassHandle::NewInstance<LibraryHandle>().As<Object>();
    auto context = Isolate::GetCurrent()->GetCurrentContext();
    Unmaybe(library->Set(context, v8_symbol("lib"),
                         ClassHandle::NewInstance<LibHandle>()));

    // Freeze prototypes of all classes
    auto prototype = HandleCast<Local<String>>("prototype");
    auto freeze = [&](const char* name) {
      auto fn = Unmaybe(library->Get(context, HandleCast<Local<String>>(name)))
                    .As<Function>();
    ...
    library->GetPrototype().As<Object>()->SetIntegrityLevel(
        context, IntegrityLevel::kFrozen);
    return library;
  }
};
...
// TODO(?): This is pointer / new is here so that these dtors don't get called
// when the module is being torn down. They end up invoking a bunch of v8
// functions which fail because nodejs already shut down the platform.
struct IsolateHolderAndJoin {
  std::shared_ptr<IsolateHolder> holder;
  std::shared_ptr<IsolateDisposeWait> dispose_wait;
};
auto* default_isolates =
    new lockable_t<std::unordered_map<v8::Isolate*, IsolateHolderAndJoin>>;
extern "C" void init(Local<Object> target) {
  ...
  // Maybe this would happen if you include the module from `vm`?
  {
    ...
    isolates->insert(std::make_pair(
        isolate, IsolateHolderAndJoin{
                     holder, holder->GetIsolate()->GetDisposeWaitHandle()}));
  }
  ...
  node::AddEnvironmentCleanupHook(
      ...
      [](void* param) {
        ...
        default_isolates->write()->erase(
            isolate);  // `it` might have changed, don't use it
      },
      isolate);

  if (!did_global_init.exchange(true)) {
    // These flags will override limits set through code. Since the main node
    // isolate is already created we can reset these so they won't affect the
    // isolates we make.
    int argc = 4;
    const char* flags[] = {"--max-semi-space-size", "0", "--max-old-space-size",
                           "0"};
