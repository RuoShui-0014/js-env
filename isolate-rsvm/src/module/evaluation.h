...
/**
 * Parses script origin information from an option object and returns a non-v8
 * holder for the information which can then be converted to a ScriptOrigin,
 * perhaps in a different isolate from the one it was read in.
 */
class ScriptOriginHolder {
 public:
  explicit ScriptOriginHolder(v8::MaybeLocal<v8::Object> maybe_options,
...
/**
 * Parser and holder for all common v8 compilation information like code string,
 * cached data, script origin, etc.
 */
class CodeCompilerHolder {
 public:
  CodeCompilerHolder(v8::Local<v8::String> code_handle,
  ...
  auto ShouldProduceCachedData() const {
    return produce_cached_data &&
           (!supplied_cached_data || cached_data_rejected);
  }
...
/**
 * Run a lambda which invokes the v8 compiler and annotate the exception with
 * source / line number if it throws.
 */
...
auto RunWithAnnotatedErrors(Function fn) {
  ...
  try {
    ...
    try {
      ...
      std::string message_str = HandleCast<std::string>(
          Unmaybe(error.As<v8::Object>()->Get(context, message_key)));
      Unmaybe(error.As<v8::Object>()->Set(
          context, message_key,
          HandleCast<v8::Local<v8::String>>(message_str + " [" + decorator +
                                            "]")));
