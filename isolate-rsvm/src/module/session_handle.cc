...
class SessionImpl : public InspectorSession {
 public:
  shared_ptr<IsolateHolder>
      isolate;  // This is the isolate that owns the session
  RemoteHandle<Function> onNotification;
  RemoteHandle<Function> onResponse;

  explicit SessionImpl(IsolateEnvironment& isolate)
      : InspectorSession(isolate) {}
 ...
 private:
  ...
  static auto bufferToString(StringBuffer& buffer) -> MaybeLocal<String> {
    ...
    if (view.is8Bit()) {
      return String::NewFromOneByte(Isolate::GetCurrent(), view.characters8(),
                                    NewStringType::kNormal, view.length());
    } else {
      return String::NewFromTwoByte(Isolate::GetCurrent(), view.characters16(),
                                    NewStringType::kNormal, view.length());
    }
  }
  ...
  void sendResponse(int call_id, unique_ptr<StringBuffer> message) final {
    ...
    struct SendResponseTask : public Runnable {
      ...
      void Run() final {
        ...
        if (bufferToString(*message).ToLocal(&string)) {
          ...
          argv[1] = string;
          try {
            Unmaybe(fn->Call(isolate->GetCurrentContext(), Undefined(isolate),
                             2, argv));
    ...
    isolate->ScheduleTask(std::make_unique<SendResponseTask>(
                              call_id, std::move(message), onResponse),
                          false, true);
  }
  ...
  void sendNotification(unique_ptr<StringBuffer> message) final {
    ...
    struct SendNotificationTask : public Runnable {
      unique_ptr<StringBuffer> message;
      RemoteHandle<Function> onNotification;
      SendNotificationTask(unique_ptr<StringBuffer> message,
                           RemoteHandle<Function> onNotification)
          : message(std::move(message)),
            onNotification(std::move(onNotification)) {}
    ...
    isolate->ScheduleTask(std::make_unique<SendNotificationTask>(
                              std::move(message), onNotification),
                          false, true);
  }
 ...
 */
SessionHandle::SessionHandle(IsolateEnvironment& isolate)
    : session(std::make_shared<SessionImpl>(isolate)) {
  session->isolate = IsolateEnvironment::GetCurrentHolder();
}

auto SessionHandle::Definition() -> Local<FunctionTemplate> {
  return MakeClass(
      "Session", nullptr, "dispatchProtocolMessage",
      MemberFunction<decltype(&SessionHandle::DispatchProtocolMessage),
                     &SessionHandle::DispatchProtocolMessage>{},
      "dispose",
      MemberFunction<decltype(&SessionHandle::Dispose),
                     &SessionHandle::Dispose>{},
      "onNotification",
      MemberAccessor<decltype(&SessionHandle::OnNotificationGetter),
                     &SessionHandle::OnNotificationGetter,
                     decltype(&SessionHandle::OnNotificationSetter),
                     &SessionHandle::OnNotificationSetter>{},
      "onResponse",
      MemberAccessor<decltype(&SessionHandle::OnResponseGetter),
                     &SessionHandle::OnResponseGetter,
                     decltype(&SessionHandle::OnResponseSetter),
                     &SessionHandle::OnResponseSetter>{});
}
 ...
 * JS API methods
 */
auto SessionHandle::DispatchProtocolMessage(Local<String> message)
    -> Local<Value> {
  Isolate* isolate = Isolate::GetCurrent();
  CheckDisposed();
  String::Value v8_str{isolate, message};
  session->DispatchBackendProtocolMessage(
      std::vector<uint16_t>(*v8_str, *v8_str + v8_str.length()));
