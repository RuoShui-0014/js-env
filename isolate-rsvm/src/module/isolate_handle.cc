 ...
 */
IsolateHandle::IsolateHandleTransferable::IsolateHandleTransferable(
...
IsolateHandle::IsolateHandle(shared_ptr<IsolateHolder> isolate)
    : isolate(std::move(isolate)) {}

auto IsolateHandle::Definition() -> Local<FunctionTemplate> {
  return Inherit<TransferableHandle>(MakeClass(
      "Isolate", ConstructorFunction<decltype(&New), &New>{}, "createSnapshot",
      FreeFunction<decltype(&CreateSnapshot), &CreateSnapshot>{},
      "compileScript",
      MemberFunction<decltype(&IsolateHandle::CompileScript<1>),
                     &IsolateHandle::CompileScript<1>>{},
      "compileScriptSync",
      MemberFunction<decltype(&IsolateHandle::CompileScript<0>),
                     &IsolateHandle::CompileScript<0>>{},
      "compileModule",
      MemberFunction<decltype(&IsolateHandle::CompileModule<1>),
                     &IsolateHandle::CompileModule<1>>{},
      "compileModuleSync",
      MemberFunction<decltype(&IsolateHandle::CompileModule<0>),
                     &IsolateHandle::CompileModule<0>>{},
      "cpuTime",
      MemberAccessor<decltype(&IsolateHandle::GetCpuTime),
                     &IsolateHandle::GetCpuTime>{},
      "createContext",
      MemberFunction<decltype(&IsolateHandle::CreateContext<1>),
                     &IsolateHandle::CreateContext<1>>{},
      "createContextSync",
      MemberFunction<decltype(&IsolateHandle::CreateContext<0>),
                     &IsolateHandle::CreateContext<0>>{},
      "createInspectorSession",
      MemberFunction<decltype(&IsolateHandle::CreateInspectorSession),
                     &IsolateHandle::CreateInspectorSession>{},
      "dispose",
      MemberFunction<decltype(&IsolateHandle::Dispose),
                     &IsolateHandle::Dispose>{},
      "getHeapStatistics",
      MemberFunction<decltype(&IsolateHandle::GetHeapStatistics<1>),
                     &IsolateHandle::GetHeapStatistics<1>>{},
      "getHeapStatisticsSync",
      MemberFunction<decltype(&IsolateHandle::GetHeapStatistics<0>),
                     &IsolateHandle::GetHeapStatistics<0>>{},
      "isDisposed",
      MemberAccessor<decltype(&IsolateHandle::IsDisposedGetter),
                     &IsolateHandle::IsDisposedGetter>{},
      "referenceCount",
      MemberAccessor<decltype(&IsolateHandle::GetReferenceCount),
                     &IsolateHandle::GetReferenceCount>{},
      "wallTime",
      MemberAccessor<decltype(&IsolateHandle::GetWallTime),
                     &IsolateHandle::GetWallTime>{},
      "startCpuProfiler",
      MemberFunction<decltype(&IsolateHandle::StartCpuProfiler),
                     &IsolateHandle::StartCpuProfiler>{},
      "stopCpuProfiler",
      MemberFunction<decltype(&IsolateHandle::StopCpuProfiler<1>),
                     &IsolateHandle::StopCpuProfiler<1>>{}));
}

/**
 * Create a new Isolate. It all starts here!
 */
auto IsolateHandle::New(MaybeLocal<Object> maybe_options)
    -> unique_ptr<ClassHandle> {
  ...
  if (maybe_options.ToLocal(&options)) {
    ...
    // Set snapshot
    auto maybe_snapshot = ReadOption<MaybeLocal<Object>>(
        options, StringTable::Get().snapshot, {});
    Local<Object> snapshot_handle;
    if (maybe_snapshot.ToLocal(&snapshot_handle)) {
      auto* copy_handle =
          ClassHandle::Unwrap<ExternalCopyHandle>(snapshot_handle.As<Object>());
      if (copy_handle != nullptr) {
        ExternalCopyArrayBuffer* copy_ptr =
            dynamic_cast<ExternalCopyArrayBuffer*>(
                copy_handle->GetValue().get());
        if (copy_ptr != nullptr) {
      ...
      if (!snapshot_blob) {
        throw RuntimeTypeError(
            "`snapshot` must be an ExternalCopy to ArrayBuffer");
      }
    }
    ...
    auto maybe_handler = ReadOption<MaybeLocal<Function>>(
        options, StringTable::Get().onCatastrophicError, {});
  ...
  auto holder = IsolateEnvironment::New(memory_limit, std::move(snapshot_blob),
                                        snapshot_blob_length);
  auto env = holder->GetIsolate();
  env->GetIsolate()->SetHostInitializeImportMetaObjectCallback(
      ModuleHandle::InitializeImportMeta);
...
struct CreateContextRunner : public ThreePhaseTask {
  ...
  explicit CreateContextRunner(MaybeLocal<Object>& maybe_options) {
    enable_inspector =
        ReadOption<bool>(maybe_options, StringTable::Get().inspector, false);
    enable_rsvm =
        ReadOption<bool>(maybe_options, StringTable::Get().rsvm, false);
    enable_intercept =
        ReadOption<bool>(maybe_options, StringTable::Get().intercept, false);
  }

  void Phase2() final {
    // Use custom deleter on the shared_ptr which will notify the isolate when
    // we're probably done with this context
    ...
    if (enable_inspector && env.GetInspectorAgent() == nullptr) {
      Context::Scope context_scope{
          env.DefaultContext()};  // TODO: This is needed to throw, but is
                                  // stupid and sloppy
    ...
    Local<Context> context_handle =
        env.NewContext(enable_rsvm, enable_intercept);
    new rs::RSWindow(context_handle->Global(), env);
    if (enable_inspector) {
      if (enable_rsvm) {
        env.GetInspectorAgent()->ContextCreated(context_handle,
                                                "rs|vx:proxy0014");
      } else {
        env.GetInspectorAgent()->ContextCreated(context_handle,
                                                "<isolated-vm>");
  ...
  auto Phase3() -> Local<Value> final {
    ...
    return ClassHandle::NewInstance<ContextHandle>(std::move(context),
                                                   std::move(global));
  }
};
template <int async>
auto IsolateHandle::CreateContext(MaybeLocal<Object> maybe_options)
    -> Local<Value> {
  return ThreePhaseTask::Run<async, CreateContextRunner>(*isolate,
                                                         maybe_options);
}
...
struct CompileScriptRunner : public CodeCompilerHolder, public ThreePhaseTask {
  ...
  CompileScriptRunner(const Local<String>& code_handle,
  ...
  void Phase2() final {
    ...
    script = RemoteHandle<UnboundScript>{
        RunWithAnnotatedErrors([&isolate, &source, compile_options]() {
          return Unmaybe(ScriptCompiler::CompileUnboundScript(
              isolate, source.get(), compile_options));
        })};
  ...
  auto Phase3() -> Local<Value> final {
    ...
    Local<Object> value =
        ClassHandle::NewInstance<ScriptHandle>(std::move(script));
...
template <int async>
auto IsolateHandle::CompileScript(Local<String> code_handle,
                                  MaybeLocal<Object> maybe_options)
    -> Local<Value> {
  return ThreePhaseTask::Run<async, CompileScriptRunner>(
      *this->isolate, code_handle, maybe_options);
}
...
struct CompileModuleRunner : public CodeCompilerHolder, public ThreePhaseTask {
  ...
  CompileModuleRunner(const Local<String>& code_handle,
                      const MaybeLocal<Object>& maybe_options)
      : CodeCompilerHolder{code_handle, maybe_options, true} {
    auto maybe_meta_callback = ReadOption<MaybeLocal<Function>>(
        maybe_options, StringTable::Get().meta, {});
  ...
  void Phase2() final {
    ...
    if (ShouldProduceCachedData()) {
      ScriptCompiler::CachedData* cached_data = ScriptCompiler::CreateCodeCache(
          module_handle->GetUnboundModuleScript());
      assert(cached_data != nullptr);
      SaveCachedData(cached_data);
    }
    ...
    if (meta_callback) {
      if (meta_callback.GetSharedIsolateHolder() !=
          IsolateEnvironment::GetCurrentHolder()) {
        throw RuntimeGenericError(
            "`meta` callback must belong to entered isolate");
      }
  ...
  auto Phase3() -> Local<Value> final {
    Local<Object> value =
        ClassHandle::NewInstance<ModuleHandle>(std::move(module_info));
    WriteCompileResults(value);
    return value;
  }
};
...
template <int async>
auto IsolateHandle::CompileModule(Local<String> code_handle,
                                  MaybeLocal<Object> maybe_options)
    -> Local<Value> {
  return ThreePhaseTask::Run<async, CompileModuleRunner>(
      *this->isolate, code_handle, maybe_options);
}
...
auto IsolateHandle::CreateInspectorSession() -> Local<Value> {
  if (IsolateEnvironment::GetCurrentHolder() == isolate) {
    throw RuntimeGenericError(
        "An isolate is not debuggable from within itself");
  }
...
struct HeapStatRunner : public ThreePhaseTask {
  ...
  auto Phase3() -> Local<Value> final {
    ...
    Unmaybe(ret->Set(context, strings.total_heap_size,
                     Number::New(isolate, heap.total_heap_size())));
    Unmaybe(ret->Set(context, strings.total_heap_size_executable,
                     Number::New(isolate, heap.total_heap_size_executable())));
    Unmaybe(ret->Set(context, strings.total_physical_size,
                     Number::New(isolate, heap.total_physical_size())));
    Unmaybe(ret->Set(
        context, strings.total_available_size,
        Number::New(isolate, static_cast<double>(heap.total_available_size()) -
                                 adjustment)));
    Unmaybe(ret->Set(context, strings.used_heap_size,
                     Number::New(isolate, heap.used_heap_size())));
    Unmaybe(ret->Set(
        context, strings.heap_size_limit,
        Number::New(isolate,
                    static_cast<double>(heap.heap_size_limit()) - adjustment)));
    Unmaybe(ret->Set(context, strings.malloced_memory,
                     Number::New(isolate, heap.malloced_memory())));
    Unmaybe(ret->Set(context, strings.peak_malloced_memory,
                     Number::New(isolate, heap.peak_malloced_memory())));
    Unmaybe(ret->Set(context, strings.does_zap_garbage,
                     Number::New(isolate, heap.does_zap_garbage())));
    Unmaybe(ret->Set(context, strings.externally_allocated_size,
                     Number::New(isolate, externally_allocated_size)));
    return ret;
  }
};
template <int async>
...
auto IsolateHandle::StartCpuProfiler(v8::Local<v8::String> title)
    -> Local<Value> {
  auto env = this->isolate->GetIsolate();
...
template <int async>
auto IsolateHandle::StopCpuProfiler(v8::Local<v8::String> title)
    -> Local<Value> {
  auto env = this->isolate->GetIsolate();
 ...
 */
static auto SerializeInternalFieldsCallback(Local<Object> /*holder*/,
                                            int /*index*/,
                                            void* /*data*/) -> StartupData {
  return {nullptr, 0};
}

auto IsolateHandle::CreateSnapshot(ArrayRange script_handles,
                                   MaybeLocal<String> warmup_handle)
    -> Local<Value> {
  ...
  for (auto value : script_handles) {
    ...
    Local<Value> script = Unmaybe(
        script_handle.As<Object>()->Get(context, StringTable::Get().code));
    if (!script->IsString()) {
      throw RuntimeTypeError("`code` property is required");
    }
    scripts.emplace_back(ExternalCopyString{script.As<String>()},
                         ScriptOriginHolder{script_handle});
  }
  ExternalCopyString warmup_script;
  if (!warmup_handle.IsEmpty()) {
    warmup_script =
        ExternalCopyString{warmup_handle.ToLocalChecked().As<String>()};
  }
  ...
  shared_ptr<ExternalCopy> error;
  {
    ...
    SnapshotCreator snapshot_creator{isolate};
    {
      ...
      snapshot_creator.SetDefaultContext(
          context, {&SerializeInternalFieldsCallback, nullptr});
      FunctorRunners::RunCatchExternal(
          context,
          [&]() {
            HandleScope handle_scope(isolate);
            ...
            for (auto& script : scripts) {
              ...
              Local<UnboundScript> unbound_script;
              {
                ...
                Local<Script> compiled_script =
                    RunWithAnnotatedErrors([&context, &source]() {
                      return Unmaybe(ScriptCompiler::Compile(
                          context, &source, ScriptCompiler::kNoCompileOptions));
                    });
              ...
              if (warmup_script) {
                ...
                Unmaybe(
                    unbound_script->BindToCurrentContext()->Run(context_dirty));
              }
            }
            if (warmup_script) {
              Context::Scope context_scope{context_dirty};
              ...
              ScriptCompiler::Source source{
                  warmup_script.CopyInto().As<String>(),
                  ScriptOrigin{script_origin}};
              RunWithAnnotatedErrors([&context_dirty, &source]() {
                Unmaybe(Unmaybe(ScriptCompiler::Compile(
                                    context_dirty, &source,
                                    ScriptCompiler::kNoCompileOptions))
                            ->Run(context_dirty));
              });
            }
    ...
    // nb: Snapshot must be created even in the error case, because
    // `~SnapshotCreator` will crash if you don't
    snapshot = snapshot_creator.CreateBlob(
  ...
  auto buffer = std::make_shared<ExternalCopyArrayBuffer>((void*)snapshot.data,
                                                          snapshot.raw_size);
