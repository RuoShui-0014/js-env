...
// Responsible for waiting for the promise in the isolate in which it was
// created
class TransferablePromiseHolder final : public ClassHandle {
 public:
  ...
  ~TransferablePromiseHolder() final {
    Save(true, [&]() {
      return std::make_shared<ExternalCopyError>(
          ExternalCopyError::ErrorType::Error, "Promise was abandoned");
    });
  }
    ...
    if (state == Promise::PromiseState::kPending) {
      ...
      promise = Unmaybe(promise->Then(
          ...
          Unmaybe(Function::New(
              context,
              FreeFunctionWithData<decltype(&Resolved), &Resolved>{}.callback,
              handle))));
      Unmaybe(promise->Catch(
          context,
          Unmaybe(Function::New(
              context,
              FreeFunctionWithData<decltype(&Rejected), &Rejected>{}.callback,
              handle))));
    } else {
 ...
 private:
  ...
  void Save(bool did_throw, Function callback) {
    ...
    auto pending_tasks =
        [&]() -> std::deque<RemoteTuple<Promise::Resolver, v8::Context>> {
    ...
    for (auto& resolver : pending_tasks) {
      ...
      holder->ScheduleTask(std::make_unique<ResolveTask>(
                               std::move(resolver), resolved_value, did_throw),
                           false, true);
    }
  }

  static void Rejected(TransferablePromiseHolder& that, Local<Value> value) {
    that.Save(true, [&]() { return ExternalCopy::CopyThrownValue(value); });
  }

  struct ResolveTask : Runnable {
    ResolveTask(RemoteTuple<Promise::Resolver, v8::Context> resolver,
                std::shared_ptr<Transferable> value,
                bool did_throw)
...
class TransferablePromise : public Transferable {
 ...
 private:
  auto MakeHolder(TransferOptions transfer_options)
      -> TransferablePromiseHolder& {
    transfer_options.promise = false;
    auto holder = ClassHandle::NewInstance<TransferablePromiseHolder>(
        state, transfer_options);
...
TransferOptions::TransferOptions(Local<Object> options, Type fallback)
    : fallback{fallback} {
  ParseOptions(options);
}

TransferOptions::TransferOptions(MaybeLocal<Object> maybe_options,
                                 Type fallback)
    : fallback{fallback} {
...
void TransferOptions::TransferOptions::ParseOptions(Local<Object> options) {
  ...
  bool externalCopy =
      ReadOption<bool>(options, StringTable::Get().externalCopy, false);
  bool reference =
      ReadOption<bool>(options, StringTable::Get().reference, false);
  if ((copy && externalCopy) || (copy && reference) ||
      (externalCopy && reference)) {
...
auto OptionalTransferOut(Local<Value> value, TransferOptions options)
    -> std::unique_ptr<Transferable> {
  auto TransferWithType =
      [&](TransferOptions::Type type) -> std::unique_ptr<Transferable> {
    switch (type) {
      default:
        return nullptr;
      ...
      case TransferOptions::Type::ExternalCopy:
        return std::make_unique<ExternalCopyHandle::ExternalCopyTransferable>(
            ExternalCopy::Copy(value));
  ...
  if (options.promise) {
    if (value->IsPromise()) {
      return std::make_unique<TransferablePromise>(value.As<Promise>(),
                                                   options);
    } else {
...
auto TransferOut(Local<Value> value, TransferOptions options)
    -> std::unique_ptr<Transferable> {
