...
auto ScriptHandle::Definition() -> Local<FunctionTemplate> {
  return Inherit<TransferableHandle>(MakeClass(
      "Script", nullptr, "release",
      MemberFunction<decltype(&ScriptHandle::Release),
                     &ScriptHandle::Release>{},
      "run",
      MemberFunction<decltype(&ScriptHandle::Run<1>), &ScriptHandle::Run<1>>{},
      "runIgnored",
      MemberFunction<decltype(&ScriptHandle::Run<2>), &ScriptHandle::Run<2>>{},
      "runSync",
      MemberFunction<decltype(&ScriptHandle::Run<0>),
                     &ScriptHandle::Run<0>>{}));
}
...
struct RunRunner /* lol */ : public ThreePhaseTask {
  RunRunner(RemoteHandle<UnboundScript>& script,
  ...
  void Phase2() final {
    ...
    Local<Value> script_result =
        RunWithTimeout(timeout_ms, [&script_handle, &context_local]() {
...
template <int async>
auto ScriptHandle::Run(ContextHandle& context_handle,
                       MaybeLocal<Object> maybe_options) -> Local<Value> {
  return ThreePhaseTask::Run<async, RunRunner>(
      *script.GetIsolateHolder(), script, context_handle, maybe_options);
}
