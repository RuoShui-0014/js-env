...
class ContextHandleTransferable : public Transferable {
 public:
  ContextHandleTransferable(RemoteHandle<Context> context,
 ...
 */
ContextHandle::ContextHandle(RemoteHandle<Context> context,
                             RemoteHandle<Value> global)
    : context{std::move(context)}, global{std::move(global)} {}

auto ContextHandle::Definition() -> Local<FunctionTemplate> {
  return Inherit<TransferableHandle>(
      MakeClass("Context", nullptr, "eval",
                MemberFunction<decltype(&ContextHandle::Eval<1>),
                               &ContextHandle::Eval<1>>{},
                "evalIgnored",
                MemberFunction<decltype(&ContextHandle::Eval<2>),
                               &ContextHandle::Eval<2>>{},
                "evalSync",
                MemberFunction<decltype(&ContextHandle::Eval<0>),
                               &ContextHandle::Eval<0>>{},
                "evalClosure",
                MemberFunction<decltype(&ContextHandle::EvalClosure<1>),
                               &ContextHandle::EvalClosure<1>>{},
                "evalClosureIgnored",
                MemberFunction<decltype(&ContextHandle::EvalClosure<2>),
                               &ContextHandle::EvalClosure<2>>{},
                "evalClosureSync",
                MemberFunction<decltype(&ContextHandle::EvalClosure<0>),
                               &ContextHandle::EvalClosure<0>>{},
                "global",
                MemberAccessor<decltype(&ContextHandle::GlobalGetter),
                               &ContextHandle::GlobalGetter>{},
                "release",
                MemberFunction<decltype(&ContextHandle::Release),
...
auto ContextHandle::GlobalGetter() -> Local<Value> {
  ...
  if (global_reference) {
    ...
    ref = ClassHandle::NewInstance<ReferenceHandle>(
        global.GetSharedIsolateHolder(), global, context,
        ReferenceHandle::TypeOf::Object, false, false);
    global_reference = RemoteHandle<v8::Object>(ref);
  }
  Unmaybe(This()->CreateDataProperty(isolate->GetCurrentContext(),
                                     StringTable::Get().global, ref));
  return ref;
}
...
auto ContextHandle::Release() -> Local<Value> {
  return Boolean::New(Isolate::GetCurrent(), [&]() {
    if (context) {
      ...
      global = {};
      if (global_reference) {
        ClassHandle::Unwrap<ReferenceHandle>(Deref(global_reference))
            ->Release();
        global_reference = {};
      }
      return true;
...
class EvalRunner : public CodeCompilerHolder, public ThreePhaseTask {
 public:
  explicit EvalRunner(RemoteHandle<Context> context,
                      Local<String> code,
    ...
    timeout_ms = ReadOption<int32_t>(maybe_options, StringTable::Get().timeout,
                                     timeout_ms);
  }
  ...
  void Phase2() final {
    ...
    Local<Value> script_result =
        RunWithTimeout(timeout_ms, [&]() { return script->Run(context); });
    result = OptionalTransferOut(script_result, transfer_options);
    heap_check.Epilogue();
  }

  auto Phase3() -> Local<Value> final {
    return result ? result->TransferIn()
...
template <int Async>
auto ContextHandle::Eval(Local<String> code, MaybeLocal<Object> maybe_options)
    -> Local<Value> {
  return ThreePhaseTask::Run<Async, EvalRunner>(*context.GetIsolateHolder(),
                                                context, code, maybe_options);
}
...
class EvalClosureRunner : public CodeCompilerHolder, public ThreePhaseTask {
 public:
  explicit EvalClosureRunner(RemoteHandle<Context> context,
                             Local<String> code,
                             Maybe<ArrayRange> maybe_arguments,
                             MaybeLocal<Object> maybe_options)
      : CodeCompilerHolder{code, maybe_options},
        transfer_options{
            ReadOption<MaybeLocal<Object>>(maybe_options,
                                           StringTable::Get().result,
                                           {})},
        argv{[&]() {
          // Transfer arguments out of isolate
          std::vector<std::unique_ptr<Transferable>> argv;
          TransferOptions transfer_options{ReadOption<MaybeLocal<Object>>(
              maybe_options, StringTable::Get().arguments, {})};
  ...
  void Phase2() final {
    ...
    for (size_t ii = 0; ii < argc; ++ii) {
      argument_names.emplace_back(
          HandleCast<Local<String>>(std::string{"$"} + std::to_string(ii)));
    }
    ...
    auto function = RunWithAnnotatedErrors([&]() {
      return Unmaybe(ScriptCompiler::CompileFunction(
          context, source.get(), argument_names.size(),
          argument_names.empty() ? nullptr : &argument_names[0], 0, nullptr));
    });
    ...
    Local<Value> script_result = RunWithTimeout(timeout_ms, [&]() {
      return function->Call(
          context, context->Global(), argv_transferred.size(),
          argv_transferred.empty() ? nullptr : &argv_transferred[0]);
    });
...
template <int Async>
auto ContextHandle::EvalClosure(Local<String> code,
                                Maybe<ArrayRange> maybe_arguments,
                                MaybeLocal<Object> maybe_options)
    -> Local<Value> {
  return ThreePhaseTask::Run<Async, EvalClosureRunner>(
      *context.GetIsolateHolder(), context, code, maybe_arguments,
      maybe_options);
}
