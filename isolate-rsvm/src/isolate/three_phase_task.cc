 ...
 */
ThreePhaseTask::CalleeInfo::CalleeInfo(Local<Promise::Resolver> resolver,
                                       Local<Context> context,
                                       Local<StackTrace> stack_trace)
    : remotes(resolver, context, stack_trace) {
  auto& env = IsolateEnvironment::GetCurrent();
  if (env.IsDefault()) {
    async = node::EmitAsyncInit(env.GetIsolate(), resolver->GetPromise(),
                                StringTable::Get().isolatedVm);
  }
}

ThreePhaseTask::CalleeInfo::CalleeInfo(CalleeInfo&& that) noexcept
    : remotes{std::move(that.remotes)},
      async{std::exchange(that.async, {0, 0})} {}

ThreePhaseTask::CalleeInfo::~CalleeInfo() {
  auto& env = IsolateEnvironment::GetCurrent();
  node::async_context tmp{0, 0};
  if (env.IsDefault() &&
      std::memcmp(&async, &tmp, sizeof(node::async_context)) != 0) {
    node::EmitAsyncDestroy(env.GetIsolate(), async);
  }
}

/**
 * Wrapper around node's version of the same class which does nothing if this
 * isn't the node isolate.
 *
 * nb: CallbackScope sets up a v8::TryCatch so if you need to catch an exception
 * do this *before* the v8::TryCatch.
 */
struct CallbackScope {
  ...
  CallbackScope(node::async_context async, Local<Object> resource) {
    auto& env = IsolateEnvironment::GetCurrent();
    if (env.IsDefault()) {
      scope = std::make_unique<node::CallbackScope>(env.GetIsolate(), resource,
                                                    async);
    }
  }
};
 ...
 */
ThreePhaseTask::Phase2Runner::Phase2Runner(unique_ptr<ThreePhaseTask> self,
                                           CalleeInfo info)
    : self(std::move(self)), info(std::move(info)) {}
...
ThreePhaseTask::Phase2Runner::~Phase2Runner() {
  if (!did_run) {
    ...
    struct Phase3Orphan : public Runnable {
      ...
      Phase3Orphan(unique_ptr<ThreePhaseTask> self, CalleeInfo info)
          : self(std::move(self)), info(std::move(info)) {}
      ...
      void Run() final {
        ...
        // Throw from promise
        Local<Object> error =
            Exception::Error(StringTable::Get().isolateIsDisposed).As<Object>();
    ...
    holder->ScheduleTask(
        std::make_unique<Phase3Orphan>(std::move(self), std::move(info)), false,
        true);
  }
}
...
void ThreePhaseTask::Phase2Runner::Run() {
  ...
  struct Phase3Failure : public Runnable {
    ...
    Phase3Failure(unique_ptr<ThreePhaseTask> self,
                  CalleeInfo info,
                  unique_ptr<ExternalCopy> error)
        : self(std::move(self)),
          info(std::move(info)),
    ...
    void Run() final {
      ...
      if (error) {
        ...
        rejection = Exception::Error(
            v8_string("An exception was thrown. Sorry I don't know more."));
      }
      if (rejection->IsObject()) {
        StackTraceHolder::ChainStack(rejection.As<Object>(),
                                     info.remotes.Deref<2>());
      }
  ...
  struct Phase3Success : public Runnable {
    ...
    Phase3Success(unique_ptr<ThreePhaseTask> self, CalleeInfo info)
        : self(std::move(self)), info(std::move(info)) {}
    ...
    void Run() final {
      ...
      FunctorRunners::RunCatchValue(
          ...
          [&](Local<Value> error) {
            // Error was thrown
            if (error->IsObject()) {
              StackTraceHolder::AttachStack(error.As<Object>(),
                                            info.remotes.Deref<2>());
            }
            Unmaybe(promise_local->Reject(context_local, error));
          });
  ...
  auto schedule_error = [&](std::unique_ptr<ExternalCopy> error) {
    // Schedule a task to enter the first isolate so we can throw the error at
    // the promise
    auto* holder = info.remotes.GetIsolateHolder();
    holder->ScheduleTask(
        std::make_unique<Phase3Failure>(std::move(self), std::move(info),
                                        std::move(error)),
        false, true);
  };
  FunctorRunners::RunCatchExternal(
      ...
      [&]() {
        ...
        if (epilogue_error) {
          ...
          auto* holder = info.remotes.GetIsolateHolder();
          holder->ScheduleTask(
              std::make_unique<Phase3Success>(std::move(self), std::move(info)),
              false, true);
        }
      },
      schedule_error);
 ...
 */
ThreePhaseTask::Phase2RunnerIgnored::Phase2RunnerIgnored(
    unique_ptr<ThreePhaseTask> self)
    : self(std::move(self)) {}
 ...
 */
auto ThreePhaseTask::RunSync(IsolateHolder& second_isolate, bool allow_async)
    -> Local<Value> {
  ...
  if (second_isolate_ref->GetIsolate() == Isolate::GetCurrent()) {
    if (allow_async) {
      throw RuntimeGenericError(
          "This function may not be called from the default thread");
    }
    // Shortcut when calling a sync method belonging to the currently entered
    // isolate. This avoids the deadlock protection below
    Phase2();
    ...
    if (Executor::IsDefaultThread() || is_recursive) {
      ...
      unique_ptr<ExternalCopy> error;
      {
        ...
        FunctorRunners::RunCatchExternal(
            second_isolate_ref->DefaultContext(),
            [&]() {
            ...
            [&](unique_ptr<ExternalCopy> error_inner) {
              // We need to stash the error in the outer unique_ptr because the
              // executor lock is still up
      ...
      if (error) {
        ...
        if (error_copy->IsObject()) {
          StackTraceHolder::ChainStack(
              error_copy.As<Object>(),
              StackTrace::CurrentStackTrace(isolate, 10));
        }
      ...
      // In this case we asyncronously call the default thread and suspend this
      // thread
      struct AsyncRunner final : public Runnable {
        ...
        AsyncRunner(ThreePhaseTask& self,
                    Scheduler::AsyncWait& wait,
                    lockable_t<bool, false, true>& done,
                    bool allow_async,
                    unique_ptr<ExternalCopy>& error)
        ...
        ~AsyncRunner() final {
          if (!did_run) {
            error = std::make_unique<ExternalCopyError>(
                ExternalCopyError::ErrorType::Error, "Isolate is disposed");
          }
          // nb: The lock must be held while invoking `notify_one` since the
          // condition variable will be destroyed immediately following the
          // wake.
      ...
      unique_ptr<ExternalCopy> error;
      {
        ...
        // Run it and sleep
        second_isolate.ScheduleTask(std::make_unique<AsyncRunner>(
                                        *this, wait, done, allow_async, error),
                                    false, true);
      ...
      throw RuntimeGenericError(
          "Calling a synchronous isolated-vm function on a non-default isolate "
          "from within an asynchronous isolated-vm function is not allowed.");
    }
  }
