...
// gcc 5's std::queue implementation has an explicit default ctor so
// std::exchange(..., {}) doesn't work. This changed in C++11 and I guess
// they're behind.
template <class Type>
...
/**
 * Keeps track of tasks an isolate needs to run and manages its run state
 * (running or waiting). This does all the interaction with libuv async and the
 * thread pool.
 */
class UvScheduler;
class Scheduler {
 ...
 public:
  ...
  void DoneRunning();
  // Request an interrupt in this isolate. `status` must == Running to invoke
  // this.
  void InterruptIsolate();
  // Interrupts an isolate running in the default thread
  void InterruptSyncIsolate();
  // Returns true if a wake was scheduled, false if the isolate is already
  // running.
  auto WakeIsolate(std::shared_ptr<IsolateEnvironment> isolate_ptr) -> bool;

  // Scheduler::AsyncWait will pause the current thread until woken up by
  // another thread
  class AsyncWait {
   public:
    enum State { pending, finished, canceled };

    // This is templated so we can cast to the protected base class here instead
    // of at the call site
    template <class Type>
    explicit AsyncWait(Type& scheduler) : scheduler{scheduler} {
...
class LockedScheduler : protected Scheduler,
 ...
 public:
  ...
  static void DecrementUvRefForIsolate(
      const std::shared_ptr<IsolateHolder>& holder);
  static void IncrementUvRefForIsolate(
      const std::shared_ptr<IsolateHolder>& holder);
};

class IsolatedScheduler final : public LockedScheduler {
 public:
  explicit IsolatedScheduler(IsolateEnvironment& env,
