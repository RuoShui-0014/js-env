 ...
 * - Phase 2 [Isolate 2]: copy data into new isolate, run work, copy data out of
 * isolate
 * - Phase 3 [Isolate 1]: copy results from phase 2 into the original isolate
 *
 * This class handles the locking and thread synchronization for either
 * synchronous or asynchronous functions. That way the same code can be used for
 * both versions of each function.
 *
 * These runners are invoked via: ThreePhaseTask::Run<async, T>(isolate,
 * args...);
 *
 * Where:
 *   async = 0 -- Synchronous execution
 *   async = 1 -- Asynchronous execution, promise returned
 *   async = 2 -- Asynchronous execution, result ignored (Phase3() is never
 * called) async = 4 -- Synchronous + asyncronous, original thread waits for
 * async Phase2()
 */
class ThreePhaseTask {
 private:
  /**
   * Contains references back to the original isolate which will be used after
   * phase 2 to wake the isolate up and begin phase 3
   */
  struct CalleeInfo {
    RemoteTuple<v8::Promise::Resolver, v8::Context, v8::StackTrace> remotes;
    node::async_context async{0, 0};
    CalleeInfo(v8::Local<v8::Promise::Resolver> resolver,
  ...
  struct Phase2Runner final : public Runnable {
    ...
    Phase2Runner(std::unique_ptr<ThreePhaseTask> self, CalleeInfo info);
  ...
  auto RunSync(IsolateHolder& second_isolate, bool allow_async)
      -> v8::Local<v8::Value>;
 ...
 public:
  ...
  static auto Run(IsolateHolder& second_isolate, Args&&... args)
      -> v8::Local<v8::Value> {
    if (async == 1) {  // Full async, promise returned
      ...
      FunctorRunners::RunCatchValue(
          ...
          [&](v8::Local<v8::Value> error) {
            ...
            if (error->IsObject()) {
              StackTraceHolder::AttachStack(error.As<v8::Object>(),
                                            stack_trace);
            }
      ...
      // Schedule Phase2 async
      second_isolate.ScheduleTask(
          std::make_unique<Phase2RunnerIgnored>(std::make_unique<T>(
              std::forward<Args>(args)...)  // <-- Phase1 / ctor called here
                                                ),
          false, true);
