...
auto IVMCpuProfile::BuildCpuProfile(Isolate* iso) -> Local<Value> {
  ...
  Unmaybe(
      profileObject->Set(context, strings.startTime,
                         Number::New(iso, static_cast<double>(start_time))));
  Unmaybe(profileObject->Set(context, strings.endTime,
                             Number::New(iso, static_cast<double>(end_time))));

  const Local<Array> samplesArr = Array::New(iso);
  const Local<Array> timeDeltasArr = Array::New(iso);

  for (int64_t i = 0; i < samples_count; i++) {
    Unmaybe(samplesArr->Set(context, i,
                            Number::New(iso, static_cast<double>(samples[i]))));
    Unmaybe(timeDeltasArr->Set(
        context, i,
        Number::New(iso, static_cast<double>(timestamps[i] - start_time))));
  }
  ...
  const Local<Array> nodeArr =
      Array::New(iso, static_cast<int>(profileNodes.size()));
...
auto IVMCpuProfile::ToJSObject(Isolate* iso) -> Local<Value> {
  ...
  Unmaybe(result->Set(context, (v8::Local<v8::Value>)strings.threadId,
                      GetTidValue(iso)));
...
auto IVMCpuProfile::CallFrame::ToJSObject(v8::Isolate* iso) -> Local<Value> {
  ...
  Unmaybe(
      callFrame->Set(context, strings.functionName,
                     String::NewFromUtf8(iso, function_name).ToLocalChecked()));
  Unmaybe(callFrame->Set(context, strings.url,
                         String::NewFromUtf8(iso, url).ToLocalChecked()));
  Unmaybe(
      callFrame->Set(context, strings.scriptId, Number::New(iso, script_id)));
  Unmaybe(callFrame->Set(context, strings.lineNumber,
                         Number::New(iso, line_number)));
  Unmaybe(callFrame->Set(context, strings.columnNumber,
                         Number::New(iso, column_number)));
  return callFrame;
}
...
auto IVMCpuProfile::ProfileNode::ToJSObject(Isolate* iso) -> Local<Value> {
  ...
  if (strlen(bailout_reason) > 0) {
    Unmaybe(nodeObj->Set(
        context, strings.bailoutReason,
        String::NewFromUtf8(iso, bailout_reason).ToLocalChecked()));
  }
  ...
  for (int j = 0; j < children_count; j++) {
    Unmaybe(childrenArr->Set(
        context, j, Number::New(iso, static_cast<double>(children[j]))));
  }
...
auto CpuProfileManager::StopProfiling(const char* title)
    -> std::vector<IVMCpuProfile> {
  const std::lock_guard<std::mutex> lock(mutex);
...
auto CpuProfileManager::InjestCpuProfile(
    const std::shared_ptr<v8::CpuProfile>& cpuProfile) -> void {
  const std::lock_guard<std::mutex> lock(mutex);
