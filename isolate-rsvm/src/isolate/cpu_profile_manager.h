...
class IVMCpuProfile {
 ...
 private:
  ...
  void FlatNodes(
      ...
      std::vector<ProfileNode>* nodes) {  // NOLINT(misc-no-recursion)
...
class CpuProfileManager {
 public:
  ...
  auto InjestCpuProfile(const std::shared_ptr<v8::CpuProfile>& cpuProfile)
      -> void;
 ...
 private:
  ...
  std::unordered_map<std::string, std::list<IVMCpuProfile>::iterator>
      profile_begin_ptrs{};
  std::mutex mutex;
};
}  // namespace ivm
