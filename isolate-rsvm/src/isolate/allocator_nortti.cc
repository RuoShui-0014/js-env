...
class ExternalMemoryHandle {
 public:
  ExternalMemoryHandle(Local<Object> local_handle, size_t size)
    ...
    handle.SetWeak(reinterpret_cast<void*>(this), &WeakCallbackV8,
                   WeakCallbackType::kParameter);
    IsolateEnvironment::GetCurrent().AddWeakCallback(&handle, WeakCallback,
                                                     this);
  }
...
/**
 * ArrayBuffer::Allocator that enforces memory limits. The v8 documentation
 * specifically says that it's unsafe to call back into v8 from this class but I
 * took a look at GetHeapStatistics() and I think it'll be ok.
 */
auto LimitedAllocator::Check(const size_t length) -> bool {
  if (v8_heap + env.extra_allocated_memory + length > next_check) {
    ...
    if (v8_heap + env.extra_allocated_memory + length >
        limit + env.misc_memory_size) {
  ...
  return v8_heap + env.extra_allocated_memory + length <=
         limit + env.misc_memory_size;
}

LimitedAllocator::LimitedAllocator(IsolateEnvironment& env, size_t limit)
    : env(env),
      limit(limit),
      v8_heap(1024 * 1024 * 4),
      next_check(1024 * 1024) {}
...
auto LimitedAllocator::Allocate(size_t length) -> void* {
  if (Check(length)) {
    ...
    if (length <= 64) {  // kMinAddedElementsCapacity * sizeof(uint32_t)
      // When a tiny TypedArray is created v8 will avoid calling the allocator
      // and instead just use the internal heap. This is all fine until someone
      // wants a pointer to the underlying buffer, in that case v8 will
      // "materialize" an ArrayBuffer which does invoke this allocator. If the
      // allocator refuses to return a valid pointer it will result in a hard
      // crash so we have no choice but to let this allocation succeed. Luckily
      // the amount of memory allocated is tiny and will soon be freed because
      // at the same time we terminate the isolate.
      env.extra_allocated_memory += length;
      env.Terminate();
      return std::calloc(length, 1);
    } else {
...
auto LimitedAllocator::Reallocate(void* data,
                                  size_t old_length,
                                  size_t new_length) -> void* {
  auto delta =
