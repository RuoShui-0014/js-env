...
/**
 * Executor class handles v8 locking while C++ code is running. Thread
 * syncronization is handled by v8::Locker. This also enters the isolate and
 * sets up a handle scope.
 */
class Executor {  // "En taro adun"
 ...
 private:
  class CpuTimer {
   public:
    ...
    auto Delta(const std::lock_guard<std::mutex>& /*lock*/) const
        -> std::chrono::nanoseconds;
    void Pause();
    void Resume();
  ...
  // WallTimer is also responsible for pausing the current CpuTimer before we
  // attempt to acquire the v8::Locker, because that could block in which case
  // CPU shouldn't be counted.
  class WallTimer {
   public:
 ...
 public:
  ...
  class UnpauseScope {
   public:
    explicit UnpauseScope(PauseScope& pause) : timer{pause.timer} {
      timer->Resume();
    }
  ...
  class Lock {
   ...
   private:
    // These need to be separate from `Executor::current_executor` because the
    // default isolate doesn't actually get a lock.
    Scope scope;
