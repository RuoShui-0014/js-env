...
Executor::Executor(IsolateEnvironment& env)
    : env{env},
      default_executor{*(current_executor == nullptr
                             ? (current_executor = this)
                             : &current_executor->default_executor)},
      default_thread{&default_executor == this
                         ? std::this_thread::get_id()
                         : default_executor.default_thread} {}
...
auto Executor::MayRunInlineTasks(IsolateEnvironment& env) -> bool {
  if (current_executor == &env.executor) {
    if (env.nodejs_isolate) {
      // nodejs isolates are active by default in their owned thread even if
      // there is no Scope up. This can cause problems when the GC runs and
      // invokes weak callbacks because there is no v8::HandleScope set up.
...
auto Executor::CpuTimer::Delta(const std::lock_guard<std::mutex>& /*lock*/)
    const -> std::chrono::nanoseconds {
#if USE_CLOCK_THREAD_CPUTIME_ID
  return std::chrono::duration_cast<std::chrono::nanoseconds>(
      std::chrono::steady_clock::now() - steady_time);
#else
...
auto Executor::WallTimer::Delta(const std::lock_guard<std::mutex>& /*lock*/)
    const -> std::chrono::nanoseconds {
  return std::chrono::duration_cast<std::chrono::nanoseconds>(
...
Executor::Profiler::Profiler(IsolateEnvironment& env) {
  ...
  if (env.GetCpuProfileManager()->IsProfiling()) {
    profiler =
        v8::CpuProfiler::New(isolate, v8::kDebugNaming, v8::kEagerLogging);
    const v8::Local<v8::String> title =
        v8::String::NewFromUtf8(isolate, "isolated-vm").ToLocalChecked();
...
Executor::Profiler::~Profiler() {
  ...
  auto DeleterLambda = [](v8::CpuProfile* profile) { profile->Delete(); };
  const v8::Local<v8::String> title =
      v8::String::NewFromUtf8(isolate, "isolated-vm").ToLocalChecked();
  const std::shared_ptr<v8::CpuProfile> profile{profiler->StopProfiling(title),
                                                DeleterLambda};
 ...
 */
Executor::Unlock::Unlock(IsolateEnvironment& env)
    : pause_scope{env.executor.cpu_timer}, unlocker{env.isolate} {}
