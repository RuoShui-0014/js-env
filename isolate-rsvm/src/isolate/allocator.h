...
class LimitedAllocator : public v8::ArrayBuffer::Allocator {
 ...
 public:
  ...
  auto Reallocate(void* data, size_t old_length, size_t new_length)
      -> void* final;

  // This is used by ExternalCopy when an ArrayBuffer is transferred. The memory
  // is not freed but we should no longer count it against the isolate
