...
static auto StringConcat(Isolate* isolate,
                         Local<String> left,
                         Local<String> right) -> Local<String> {
...
auto GetPrivateStackSymbol() -> Local<Private> {
  ...
  return holder.Deref([]() { return Private::New(Isolate::GetCurrent()); });
}

/**
 * Helper which renders to string either a String (pass-through), StackTrace
 * (render), or Array (recursion pair)
 */
auto RenderErrorStack(Local<Value> data) -> Local<String> {
  Isolate* isolate = Isolate::GetCurrent();
  if (data->IsString()) {
    // Plain string. We need to remove the first line of `stack` to avoid
    ...
    while (pos != nullptr) {
      if (strncmp(pos + 1, "    at", 6) == 0) {
        ...
        return Unmaybe(String::NewFromOneByte(
            isolate, reinterpret_cast<const uint8_t*>(pos),
    ...
    return StringConcat(
        isolate,
        StringConcat(isolate, RenderErrorStack(Unmaybe(array->Get(context, 1))),
                     Unmaybe(String::NewFromOneByte(
                         isolate,
    ...
    // StackTraceHolder
    StackTraceHolder& that =
        *ClassHandle::Unwrap<StackTraceHolder>(data.As<Object>());
    Local<StackTrace> stack_trace = Deref(that.stack_trace);
    return Unmaybe(String::NewFromUtf8(
        isolate, StackTraceHolder::RenderSingleStack(stack_trace).c_str(),
        NewStringType::kNormal));
  }
}
 ...
 */
void ErrorStackGetter(Local<Name> /*property*/,
  ...
  FunctorRunners::RunCallback(info, [&info]() {
    ...
    return StringConcat(
        ...
        StringConcat(isolate,
                     Unmaybe(Unmaybe(info.This()->Get(
                                         context, StringTable::Get().message))
                                 ->ToString(context)),
                     RenderErrorStack(Unmaybe(holder->GetPrivate(
                         context, GetPrivateStackSymbol())))));
  });
}
...
void AttachStackGetter(Local<Object> error, Local<Value> data) {
  ...
  Unmaybe(error->SetAccessor(
      context, StringTable::Get().stack, ErrorStackGetter, nullptr,
      Local<Value>(), AccessControl::DEFAULT, PropertyAttribute::DontEnum));
}

/**
 * StackTraceHolder implementation
 */
StackTraceHolder::StackTraceHolder(Local<StackTrace> stack_handle)
    : stack_trace(Isolate::GetCurrent(), stack_handle) {}

auto StackTraceHolder::Definition() -> Local<FunctionTemplate> {
  return MakeClass("StackTraceHolder", nullptr);
}

void StackTraceHolder::AttachStack(Local<Object> error,
                                   Local<StackTrace> stack) {
  AttachStackGetter(error, ClassHandle::NewInstance<StackTraceHolder>(stack));
}

void StackTraceHolder::ChainStack(Local<Object> error,
                                  Local<StackTrace> stack) {
  Isolate* isolate = Isolate::GetCurrent();
  Local<Context> context = isolate->GetCurrentContext();
  Local<Value> existing_data =
      Unmaybe(error->GetPrivate(context, GetPrivateStackSymbol()));
  if (existing_data->IsUndefined()) {
    ...
    if (existing_stack.IsEmpty()) {
      // In this case it's probably passed through ExternalCopy which flattens
      // the `stack` property into a plain value
      ...
      existing_data =
          ClassHandle::NewInstance<StackTraceHolder>(existing_stack);
    }
  }
  Local<Array> pair = Array::New(isolate, 2);
  Unmaybe(
      pair->Set(context, 0, ClassHandle::NewInstance<StackTraceHolder>(stack)));
  Unmaybe(pair->Set(context, 1, existing_data));
  AttachStackGetter(error, pair);
}

auto StackTraceHolder::RenderSingleStack(Local<StackTrace> stack_trace)
    -> std::string {
  Isolate* isolate = Isolate::GetCurrent();
  std::stringstream ss;
...
void StackTraceHolder::AppendFileLocation(Isolate* /*isolate*/,
                                          Local<StackFrame> frame,
                                          std::stringstream& ss) {
  String::Utf8Value script_name{Isolate::GetCurrent(),
