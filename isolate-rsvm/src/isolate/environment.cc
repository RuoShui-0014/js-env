...
static auto GetStackBase() -> void* {
  ...
  return (void*)((char*)pthread_get_stackaddr_np(self) -
...
void IsolateEnvironment::HeapCheck::Epilogue() {
  if (!env.nodejs_isolate &&
      (force || env.extra_allocated_memory != extra_size_before)) {
    ...
    if (heap.used_heap_size() + env.extra_allocated_memory > env.memory_limit) {
      ...
      isolate->GetHeapStatistics(&heap);
      if (heap.used_heap_size() + env.extra_allocated_memory >
          env.memory_limit) {
 ...
 */
void IsolateEnvironment::OOMErrorCallback(const char* location,
                                          bool is_heap_oom) {
  if (RaiseCatastrophicError(IsolateEnvironment::GetCurrent().error_handler,
                             "Catastrophic out-of-memory error")) {
    while (true) {
  ...
  fprintf(stderr,
          ...
          "does_zap_garbage = %zd\n",
          heap.total_heap_size(), heap.total_heap_size_executable(),
          heap.total_physical_size(), heap.total_available_size(),
          heap.used_heap_size(), heap.heap_size_limit(), heap.malloced_memory(),
          heap.peak_malloced_memory(), heap.does_zap_garbage());
  abort();
}
...
void IsolateEnvironment::PromiseRejectCallback(PromiseRejectMessage rejection) {
  ...
  if (event == kPromiseRejectWithNoHandler) {
    that.unhandled_promise_rejections.emplace_back(that.isolate,
                                                   rejection.GetPromise());
...
auto IsolateEnvironment::CodeGenCallback(Local<Context> /*context*/,
                                         Local<Value> source)
    -> ModifyCodeGenerationFromStringsResult {
  auto& that = IsolateEnvironment::GetCurrent();
  // This heuristic could be improved by looking up how much `timeout` this
  // isolate has left and returning early in some cases
  ModifyCodeGenerationFromStringsResult result;
  if (source->IsString() && static_cast<size_t>(source.As<String>()->Length()) >
                                static_cast<size_t>(that.memory_limit / 8)) {
    return result;
  }
...
auto IsolateEnvironment::CodeGenCallback2(Local<Context> context,
                                          Local<Value> source,
                                          bool)
    -> ModifyCodeGenerationFromStringsResult {
...
void IsolateEnvironment::MarkSweepCompactEpilogue(Isolate* isolate,
                                                  GCType /*gc_type*/,
                                                  GCCallbackFlags gc_flags,
                                                  void* data) {
  ...
  if (total_memory > memory_limit) {
    if ((gc_flags &
         (GCCallbackFlags::kGCCallbackFlagCollectAllAvailableGarbage |
          GCCallbackFlags::kGCCallbackFlagForced)) == 0) {
    ...
    if (that->did_adjust_heap_limit) {
      // There is also `AutomaticallyRestoreInitialHeapLimit` introduced in
      // v8 7.3.411 / 93283bf04 but it seems less effective than this ratcheting
      // strategy.
      isolate->RemoveNearHeapLimitCallback(NearHeapLimitCallback,
                                           that->memory_limit);
...
auto IsolateEnvironment::NearHeapLimitCallback(void* data,
                                               size_t current_heap_limit,
                                               size_t /*initial_heap_limit*/)
    -> size_t {
  // This callback will temporarily give the v8 vm up to an extra 1 GB of memory
  // to prevent the application from crashing.
  auto* that = static_cast<IsolateEnvironment*>(data);
  that->did_adjust_heap_limit = true;
  HeapStatistics heap;
  that->isolate->GetHeapStatistics(&heap);
  if (heap.used_heap_size() + that->extra_allocated_memory >
      that->memory_limit + that->misc_memory_size) {
    that->RequestMemoryPressureNotification(MemoryPressureLevel::kCritical,
                                            true);
  } else {
    that->RequestMemoryPressureNotification(MemoryPressureLevel::kModerate,
...
void IsolateEnvironment::RequestMemoryPressureNotification(
    MemoryPressureLevel memory_pressure,
    bool as_interrupt) {
  this->memory_pressure = memory_pressure;
  if (as_interrupt) {
    if (memory_pressure > last_memory_pressure) {
      isolate->RequestInterrupt(MemoryPressureInterrupt,
    ...
    if (memory_pressure == MemoryPressureLevel::kCritical) {
      ...
      MarkSweepCompactEpilogue(isolate, GCType::kGCTypeMarkSweepCompact,
...
void IsolateEnvironment::MemoryPressureInterrupt(Isolate* /*isolate*/,
                                                 void* data) {
...
void IsolateEnvironment::AsyncEntry() {
  ...
  if (!nodejs_isolate) {
    // Set v8 stack limit on non-default isolate. This is only needed on
    // non-default threads while on OS X because it allocates just 512kb for
    // each pthread stack, instead of 2mb on other systems. 512kb is lower than
    // the default v8 stack size so JS stack overflows result in segfaults.
    ...
    if (stack_base != nullptr) {
      ...
      isolate->SetStackLimit(reinterpret_cast<uintptr_t>(
          reinterpret_cast<char*>(stack_base) + 1024 * 24));
    }
  }
...
void IsolateEnvironment::IsolateCtor(size_t memory_limit_in_mb,
  ...
  size_t young_space_in_kb = (size_t)std::pow(
  ...
  task_runner =
      std::make_shared<IsolateTaskRunner>(holder.lock()->GetIsolate());
  {
  ...
  // Add GC callbacks
  isolate->AddGCEpilogueCallback(MarkSweepCompactEpilogue,
                                 static_cast<void*>(this),
                                 GCType::kGCTypeMarkSweepCompact);
  isolate->AddNearHeapLimitCallback(NearHeapLimitCallback,
                                    static_cast<void*>(this));

  // Heap statistics crushes down lots of different memory spaces into a single
  // number. We note the difference between the requested old space and v8's
  ...
  // There is no asynchronous Isolate ctor so we should throw away thread
  // specifics in case the client always uses async methods
  isolate->DiscardThreadSpecificMetadata();

  // Save reference to this isolate in the default isolate
  Executor::GetDefaultEnvironment().owned_isolates->write()->insert(
      {dispose_wait, holder});
}
...
IsolateEnvironment::~IsolateEnvironment() {
  if (nodejs_isolate) {
    ...
    }
    {
      std::lock_guard allocator_lock{isolate_allocator_mutex};
      {
        // Dispose() will call destructors for external strings and array
        // buffers, so this lock sets the "current" isolate for those C++ dtors
        // to function correctly without locking v8
    ...
    executor.default_executor.env.owned_isolates->write()->erase(
        {dispose_wait, holder});
  }
...
static void DeserializeInternalFieldsCallback(Local<Object> /*holder*/,
                                              int /*index*/,
                                              StartupData /*payload*/,
                                              void* /*data*/) {}

auto IsolateEnvironment::NewContext(bool rsvm, bool interceptor)
    -> Local<Context> {
  MaybeLocal<ObjectTemplate> global_template =
      rsvm ? rs::CreateRsGlobalTemplate(isolate, interceptor)
...
void IsolateEnvironment::Terminate() {
  ...
  isolate->RequestInterrupt(
      [](Isolate* isolate, void* /* param */) {
        isolate->AddBeforeCallEnteredCallback(
            [](Isolate* isolate) { isolate->TerminateExecution(); });
...
void IsolateEnvironment::AddWeakCallback(Persistent<Value>* handle,
                                         void (*fn)(void*),
                                         void* param) {
  if (nodejs_isolate) {
    return;
...
auto RaiseCatastrophicError(RemoteHandle<Function>& handler,
  ...
  handler.GetIsolateHolder()->ScheduleTask(
      std::make_unique<ErrorTask>(message, handler), false, true);
  return true;
}
