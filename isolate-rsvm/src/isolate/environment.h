...
/**
 * Wrapper around Isolate with helpers to make working with multiple isolates
 * easier.
 */
class IsolateEnvironment {
  // These are here so they can adjust `extra_allocated_memory`. TODO: Make this
  // a method
  ...
  template <typename F>
  friend auto RunWithTimeout(uint32_t timeout_ms, F&& fn)
      -> v8::Local<v8::Value>;
 ...
 private:
  ...
  // Another good candidate for std::optional<> (because this is only used by
  // the root isolate)
  ...
  Executor executor;
  std::shared_ptr<IsolateDisposeWait> dispose_wait{
      std::make_shared<IsolateDisposeWait>()};
  std::weak_ptr<IsolateHolder> holder;
  ...
  std::unordered_map<v8::Persistent<v8::Value>*,
 ...
 public:
  ...
  std::unordered_map<class NativeModule*, std::shared_ptr<NativeModule>>
      native_modules;
 ...
 private:
  ...
  static void OOMErrorCallback(const char* location,
  ...
  /**
   * Called when an isolate has an uncaught error in a promise. This makes no
   * distinction between contexts so we have to handle that ourselves.
   */
 ...
 private:
  static auto CodeGenCallback(v8::Local<v8::Context> context,
                              v8::Local<v8::Value> source)
      -> v8::ModifyCodeGenerationFromStringsResult;
  static auto CodeGenCallback2(v8::Local<v8::Context> context,
                               v8::Local<v8::Value> source,
                               bool)
      -> v8::ModifyCodeGenerationFromStringsResult;
   ...
   */
  static void MarkSweepCompactEpilogue(v8::Isolate* isolate,
                                       v8::GCType gc_type,
                                       v8::GCCallbackFlags gc_flags,
                                       void* data);
  static auto NearHeapLimitCallback(void* data,
                                    size_t current_heap_limit,
                                    size_t initial_heap_limit) -> size_t;
  void RequestMemoryPressureNotification(
      v8::MemoryPressureLevel memory_pressure,
      bool as_interrupt = false);
  static void MemoryPressureInterrupt(v8::Isolate* isolate, void* data);
  void CheckMemoryPressure();

  /**
   * Wrap an existing Isolate. This should only be called for the main node
   * Isolate.
   */
   ...
   */
  void IsolateCtor(size_t memory_limit_in_mb,
                   std::shared_ptr<v8::BackingStore> snapshot_blob,
 ...
 public:
   ...
   */
  static auto New(v8::Isolate* isolate, v8::Local<v8::Context> context)
      -> std::shared_ptr<IsolateHolder> {
  ...
  static auto New(size_t memory_limit_in_mb,
                  std::shared_ptr<v8::BackingStore> snapshot_blob,
                  size_t snapshot_length) -> std::shared_ptr<IsolateHolder> {
    auto env = std::make_shared<IsolateEnvironment>(static_cast<UvScheduler&>(
        *Executor::GetDefaultEnvironment().scheduler));
    auto holder = std::make_shared<IsolateHolder>(env);
    env->holder = holder;
    env->IsolateCtor(memory_limit_in_mb, std::move(snapshot_blob),
                     snapshot_length);
    return holder;
  }
  ...
  auto GetScheduler() -> LockedScheduler& { return *scheduler; }
  ...
  auto operator->() const
      -> v8::Isolate* {  // Should probably remove this one..
    return isolate;
  }

  auto GetIsolate() const -> v8::Isolate* { return isolate; }

  /**
   * Default context, useful for generating certain objects when we aren't in a
   * context.
   */
  ...
  /**
   * Creates a new context. Must be used instead of Context::New() because of
   * snapshot deserialization
   */
  auto NewContext(bool rsvm = false, bool interceptor = false)
      -> v8::Local<v8::Context>;
 ...
 public:
  ...
  /**
   * This is called after user code runs. This throws a fatal error if the
   * memory limit was hit. If an asyncronous exception (promise) was lost, this
   * will throw it for real.
   */
  auto TaskEpilogue() -> std::unique_ptr<class ExternalCopy>;

  /**
   * Get allocator used by this isolate. Will return nullptr for the default
   * isolate.
   */
  ...
  auto GetDisposeWaitHandle() { return dispose_wait; }

  /**
   * Check memory limit flag
   */
  auto DidHitMemoryLimit() const -> bool { return hit_memory_limit; }

  /**
   * Not to be confused with v8's `ExternalAllocatedMemory`. This counts up how
   * much memory this isolate is holding onto outside of v8's heap, even if that
   * memory is shared amongst other isolates.
   */
  auto GetExtraAllocatedMemory() const -> size_t {
    return extra_allocated_memory;
  }
  void AdjustExtraAllocatedMemory(int size) { extra_allocated_memory += size; }

  /**
   * Returns the current number of outstanding RemoteHandles<> to this isolate.
   */
  auto GetRemotesCount() const -> unsigned int { return remotes_count.load(); }
  void AdjustRemotes(int delta) { remotes_count.fetch_add(delta); }

  /**
   * Is this the default nodejs isolate?
   */
  auto IsDefault() const -> bool { return nodejs_isolate; }
  ...
  /**
   * Since a created Isolate can be disposed of at any time we need to keep
   * track of weak persistents to call those destructors on isolate disposal.
   */
  void AddWeakCallback(v8::Persistent<v8::Value>* handle,
                       void (*fn)(void*),
                       void* param);
...
auto IsolateSpecific<Type>::Deref(Functor callback) -> v8::Local<Type> {
  ...
  if (eternal.IsEmpty()) {
    ...
    // After `callback` is invoked `eternal` is no longer valid! The callback
    // may reference new IsolateSpecifics which will end up calling `resize` on
...
auto RaiseCatastrophicError(RemoteHandle<v8::Function>& handler,
