...
void Scheduler::InterruptIsolate() {
  assert(status == Status::Running);
  // Since this callback will be called by v8 we can be certain the pointer to
  // `isolate` is still valid
  env.GetIsolate()->RequestInterrupt(
      [](Isolate* /*isolate_ptr*/, void* env_ptr) {
...
auto Scheduler::WakeIsolate(std::shared_ptr<IsolateEnvironment> isolate_ptr)
    -> bool {
  if (status == Status::Waiting) {
    status = Status::Running;
    // Move shared reference to this scheduler to ensure the IsolateEnvironment
    // won't be deleted before a thread picks up this work.
    assert(!env_ref);
...
auto LockedScheduler::GetForegroundTaskRunner()
    -> std::shared_ptr<v8::TaskRunner> {
  return env.GetTaskRunner();
}

void LockedScheduler::DecrementUvRefForIsolate(
    const std::shared_ptr<IsolateHolder>& holder) {
...
void LockedScheduler::IncrementUvRefForIsolate(
    const std::shared_ptr<IsolateHolder>& holder) {
  auto ref = holder->GetIsolate();
  if (ref) {
...
IsolatedScheduler::IsolatedScheduler(IsolateEnvironment& env,
                                     UvScheduler& default_scheduler)
    : LockedScheduler{env}, default_scheduler{default_scheduler} {}
...
void IsolatedScheduler::SendWake() {
  thread_pool.exec(
      ...
      [](bool pool_thread, void* param) {
        ...
        // Grab reference to default scheduler, since resetting `ref` may
        // deallocate `scheduler` and invalidate the instance. Resetting `ref`
        // must take place here because the destructor might invoke cleanup
        // tasks on the default isolate which will increment `uv_ref_count`.
        // `uv_ref_count` needs to be incremented before it's decremented
        // otherwise `UvScheduler` will try to invoke `uv_ref` from a
        // non-default thread.
        auto& default_scheduler = scheduler.default_scheduler;
        ref = {};
        if (--default_scheduler.uv_ref_count == 0) {
          // Wake up the libuv loop so we can unref the async handle from the
          // default thread.
...
UvScheduler::UvScheduler(IsolateEnvironment& env)
  ...
  uv_async_init(loop, uv_async, [](uv_async_t* async) {
    ...
    auto ref = [&]() {
      // Lock is required to access env_ref on the default scheduler but a
      // non-default scheduler doesn't need it. This is because `WakeIsolate`
      // can trigger this function via `uv_async_send` while another thread is
      // writing to `env_ref`
