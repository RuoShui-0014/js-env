...
void IsolateHolder::ScheduleTask(std::unique_ptr<Runnable> task,
                                 bool run_inline,
                                 bool wake_isolate,
                                 bool handle_task) {
...
void IsolateTaskRunner::PostDelayedTask(std::unique_ptr<v8::Task> task,
                                        double delay_in_seconds) {
  // wait_detached erases the type of the lambda into a std::function which must
  // be copyable. The unique_ptr is stored in a shared pointer so ownership can
  // be handled correctly.
  auto shared_task =
      std::make_shared<std::unique_ptr<v8::Task>>(std::move(task));
  auto weak_env = this->weak_env;
  timer_t::wait_detached(
      static_cast<uint32_t>(delay_in_seconds * 1000),
      [shared_task, weak_env](void* next) {
        auto env = weak_env.lock();
        if (env) {
          // Don't wake the isolate, this will just run the next time the
          // isolate is doing something
