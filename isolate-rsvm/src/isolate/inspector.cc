 ...
 */
InspectorAgent::InspectorAgent(IsolateEnvironment& isolate)
    : isolate(isolate), inspector(V8Inspector::create(isolate, this)) {
  struct AddDefaultContext : public Runnable {
    InspectorAgent& that;
    explicit AddDefaultContext(InspectorAgent& agent) : that(agent) {}
    void Run() final {
      // We can assume that agent is still alive because otherwise this won't
      // get called
...
/**
 * When debugging running JS code this function is called and is expected to
 * block and process inspector messages until the inspector is done. This
 * happens in the isolate's currently running thread so we know that an
 * Executor::Lock is up.
 */
...
/**
 * This is the notificaiton to exit from the currently running
 * `runMessageLoopOnPause` from another thread.
 */
void InspectorAgent::quitMessageLoopOnPause() {
  // This is called from within runMessageLoopOnPause, therefore lock is already
  // acquired
  running = false;
  cv.notify_all();
}
...
/**
 * Connects a V8Inspector::Channel to a V8Inspector which returns an instance of
 * V8InspectorSession.
 */
auto InspectorAgent::ConnectSession(InspectorSession& session)
    -> unique_ptr<V8InspectorSession> {
  active_sessions.write()->insert(&session);
#if V8_AT_LEAST(10, 3, 118)
  return inspector->connect(
      1, &session, StringView(),
...
void InspectorAgent::SendInterrupt(unique_ptr<Runnable> task) {
  // If this isolate is already locked just run the task. This happens at least
  // when the isolate is disposing and `holder` no longer has a valid shared_ptr
...
void InspectorAgent::ContextCreated(Local<Context> context,
  ...
  inspector->contextCreated(
      V8ContextInfo(context, 1,
                    StringView(reinterpret_cast<const uint8_t*>(name.c_str()),
                               name.length())));
}
 ...
 */
InspectorSession::InspectorSession(IsolateEnvironment& isolate)
    : agent(*isolate.GetInspectorAgent()),
      session(agent.ConnectSession(*this)) {}
...
/**
 * Disconnects this session from the v8 inspector. Allows inspector to
...
void InspectorSession::Disconnect() {
  ...
  if (session) {
    ...
    struct DeleteSession : public Runnable {
      ...
      explicit DeleteSession(shared_ptr<V8InspectorSession> session)
          : session(std::move(session)) {}
      // Nothing is needed since the dtor will (should) delete the session in
      // the right context
      void Run() final {}
    };
 ...
 */
void InspectorSession::DispatchBackendProtocolMessage(
  ...
  if (session) {
    struct DispatchMessage : public Runnable {
      ...
      explicit DispatchMessage(std::vector<uint16_t> message,
                               shared_ptr<V8InspectorSession>& session)
          : message(std::move(message)), weak_session(session) {}
      void Run() final {
        auto session = weak_session.lock();
        if (session) {
          session->dispatchProtocolMessage(
              StringView(&message[0], message.size()));
        }
      }
    };
    agent.SendInterrupt(
        std::make_unique<DispatchMessage>(std::move(message), session));
  } else {
